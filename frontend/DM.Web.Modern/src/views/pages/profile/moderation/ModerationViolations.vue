...
import type {
  ...
  CreateWarning,
  CreateBan,
} from "@/api/requests/moderationApi";
import { BanType } from "@/api/requests/moderationApi";
...
async function toggleWarnings() {
  ...
  if (showWarnings.value && warningsList.value === null) {
    ...
    const { data } = await moderationApi.getWarnings(props.targetLogin as UserLogin);
    warningsList.value = data?.resource?.warnings ?? [];
...
async function toggleBans() {
  ...
  if (showBans.value && bansList.value === null) {
    ...
    const { data } = await moderationApi.getBans(props.targetLogin as UserLogin);
    bansList.value = data?.resource?.history ?? [];
        ...
        <template v-else-if="warningsList">
          ...
          >
            <div class="mod-violation-item_header">
              ...
              <span v-if="w.moderator">{{ w.moderator.login }}</span>
        ...
        <template v-else-if="bansList">
          ...
          >
            <div class="mod-violation-item_header">
              ...
              <span v-if="b.moderator">{{ b.moderator.login }}</span>
