...
export default new (class ChatApi {
  ...
  public getLastMessageOnOrBeforeDate(date: string) {
    return Api.get<Envelope<ChatMessage>>(`globalchat/logs/${date}/last`);
  }

  public getMessagesBefore(messageId: string, count: number = 50) {
    return Api.get<ListEnvelope<ChatMessage>>(
      `globalchat/messages/${messageId}/before`,
      { count },
    );
  }
  ...
  public getMessagesAfter(messageId: string, count: number = 50) {
    return Api.get<ListEnvelope<ChatMessage>>(
      `globalchat/messages/${messageId}/after`,
      { count },
    );
  }
  ...
  public getMessagesAround(messageId: string, count: number = 50) {
    return Api.get<ListEnvelope<ChatMessage>>(
      `globalchat/messages/${messageId}/around`,
      { count },
    );
  }
  ...
  public getMessageForEdit(id: string) {
    return Api.get<Envelope<ChatMessage>>(
      `globalchat/messages/${id}`,
      undefined,
      BbRenderMode.Bb,
    );
  }
  ...
  public updateMessage(id: string, text: string) {
    return Api.patch<Envelope<ChatMessage>>(`globalchat/messages/${id}`, {
      text,
    });
  }
