...
export const useChatStore = defineStore("chat", () => {
  ...
  // Navigate to messages for a specific date (or nearest before if empty)
  async function navigateToDate(date: string) {
    loading.value = true;
    try {
      // Use the endpoint that finds last message on or before the date
      const { data } = await chatApi.getLastMessageOnOrBeforeDate(date);
