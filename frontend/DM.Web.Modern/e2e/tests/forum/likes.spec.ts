...
test.describe('Likes', () => {
  test.skip('should like topic when authenticated', async ({ authenticatedPage }) => {
    ...
    const firstTopic = authenticatedPage.locator('.topics-row .col-title a').first();
    if (await firstTopic.isVisible()) {
      await firstTopic.click();
      // Look for like button on topic page
      const likeButton = authenticatedPage.locator('[data-testid="like-button"], .like-button');
