<template>
  ...
  >
    <the-loader v-if="!gamesStore.requirementGames" />
    <template v-else-if="gamesStore.requirementGames.length === 0">
      <secondary-text>Пока тут ничего нет...</secondary-text>
    </template>
    <game-menu-link
      v-else
      v-for="game in gamesStore.requirementGames"
      :key="game.id"
      :game="game"
      :counters="true"
    />
...
onMounted(() => {
  ...
  gamesStore.fetchRequirementGames();
