<template>
  <div class="footer">
    <div class="credits">
      <div class="copyright-line">© DM.AM, 2007 &ndash; {{ currentYear }}. <span class="age">18+</span></div>
      <div>Основатель сайта – <router-link :to="{ name: 'profile', params: { login: 'rakot' } }">rakot</router-link></div>
      <div>Администрирование и разработка – <router-link :to="{ name: 'profile', params: { login: 'Evengard' } }">Evengard</router-link>, <router-link :to="{ name: 'profile', params: { login: 'SolohinLex' } }">SolohinLex</router-link></div>
      <div>Дизайн логотипа – <router-link :to="{ name: 'azur-profile' }">Azur</router-link></div>
    </div>
    <div class="social">
      <a href="https://vk.com/club376658" target="_blank" title="Группа ВКонтакте">
      ...
      <a href="https://discord.gg/ez7FdeYgQv" target="_blank" title="Discord сервер">
        <svg class="social-icon" viewBox="0 0 24 24" fill="currentColor">
      ...
      <a href="https://www.youtube.com/@DungFM" target="_blank" title="YouTube канал">
        <svg class="social-icon" viewBox="0 0 24 24" fill="currentColor">
...
.footer
  ...
  padding: ($medium + $small) $big $medium
...
.age
  ...
  position: relative
  top: -2px
  +theme(border, $secondary-text, 1px solid)
  +theme(color, $secondary-text)
