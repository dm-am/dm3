...
useFetchData(
  () => messagingStore.fetchConversations(extractNumberParam(route.params.n as string)),
  [
    {
      param: (p) => p.n,
      callback: (n) => messagingStore.fetchConversations(extractNumberParam(n as string)),
    },
  ]
);
  ...
  <div class="messenger-list">
    <div class="search-section">
      ...
      <div v-if="showResults && (searchResults.length > 0 || isSearching)" class="search-results">
        ...
        >
          <img :src="user.smallPictureUrl || defaultAvatar" class="result-avatar" />
          <span class="result-name">{{ user.login }}</span>
        </div>
        <div v-if="!isSearching && searchResults.length === 0 && searchQuery.length > 0" class="search-empty">
          Никого не найдено
        </div>
      </div>
    </div>
    ...
    <div v-else class="empty-state">
      <svg class="empty-icon" viewBox="0 0 64 64" width="64" height="64" fill="none" stroke="currentColor" stroke-width="1.5">
