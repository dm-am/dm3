...
export const useMessagingStore = defineStore("messaging", () => {
  ...
  async function fetchConversations(number: number = 1) {
    ...
    try {
      const size = currentUser.value?.settings?.pagingLimits?.entitiesPerPage ?? 20;
      const { data, error } = await messagingApi.getConversations({ number, size });
      if (error) {
  ...
  async function selectConversation(id: ConversationId) {
    ...
    try {
      const { data, error } = await messagingApi.getConversation(id);
      if (error) {
  ...
  async function selectDirectConversation(login: UserLogin) {
    loadingConversation.value = true;
    try {
      const { data, error } = await messagingApi.getDirectConversation(login);
      if (error) {
  ...
  async function fetchMessages(conversationId: ConversationId) {
    ...
    try {
      ...
      const { data: firstData } = await messagingApi.getMessages(conversationId, { number: 1, size: PAGE_SIZE });
      ...
      if (totalPages === 1) {
        ...
        const { data } = await messagingApi.getMessages(conversationId, { number: totalPages, size: PAGE_SIZE });
  ...
  async function fetchMoreBefore() {
    if (loadingBefore.value || !hasMoreBefore.value || !selectedConversation.value || !currentPaging.value) return;
    ...
    try {
      ...
      const { data } = await messagingApi.getMessages(selectedConversation.value.id, {
        number: prevPage,
        size: PAGE_SIZE,
      });
  ...
  const interlocutor = computed(() => {
    ...
    return selectedConversation.value.participants.find(
      (p) => p.login !== currentUser.value!.login
    ) ?? null;
  });
  ...
  async function fetchUnreadCount() {
    ...
    const { data } = await messagingApi.getConversations({ number: 1, size: 20 });
    if (data) {
  ...
  async function markAsRead(conversationId: ConversationId) {
    ...
    if (selectedConversation.value?.id === conversationId) {
      (selectedConversation.value as Conversation).unreadMessagesCount = 0 as Conversation["unreadMessagesCount"];
    }
    if (conversations.value) {
      const conv = conversations.value.resources.find((c) => c.id === conversationId);
      if (conv) {
        (conv as Conversation).unreadMessagesCount = 0 as Conversation["unreadMessagesCount"];
      }
    }
  }
  ...
  async function sendMessage(conversationId: ConversationId, text: string) {
    ...
    sending.value = true;
    try {
      const { data, error } = await messagingApi.sendMessage(conversationId, text);
      if (!error && data) {
        ...
        if (conversations.value) {
          const conv = conversations.value.resources.find((c) => c.id === conversationId);
          if (conv) {
            (conv as Conversation).lastMessage = data.resource as typeof conv.lastMessage;
          }
        }
        if (selectedConversation.value?.id === conversationId) {
          (selectedConversation.value as Conversation).lastMessage = data.resource as Conversation["lastMessage"];
        }
      }
      return data?.resource ?? null;
    } finally {
  ...
  async function updateMessage(id: string, text: string) {
    const { data, error } = await messagingApi.updateMessage(id as MessageId, { text });
    if (!error && data) {
  ...
  async function unlikeMessage(id: string) {
    ...
    if (!error && currentUser.value) {
      ...
      if (idx !== -1) {
        ...
        messagesList.value[idx] = {
          ...messagesList.value[idx],
          likes: messagesList.value[idx].likes.filter((u) => u.login !== currentLogin) as Message["likes"],
        };
      }
    }
    return { error };
