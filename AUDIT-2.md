...
Готово, когда в индексе ниже не осталось строк со статусом «Открыто», а ярус e2e дает
exit 0. Оценки срезов — следствие: срез достигает потолка, когда закрыты его строки.
...
Почему без гейта работа возвращается — измерено на первом аудите, а не предположено: из 306
закрытых находок перепроверка нашла 39 закрытых частично (правка по нескольким местам, одно
забыто); три дефекта внесены самими исправлениями; из 156 коммитов ветки 30 поведенческих
правок не тронули ни одного теста.

## Ждет решения владельца

Пока эти вопросы без ответа, свои срезы они держат ниже потолка:

- Пуш ветки: CI не запускался ни разу. Тег pre-squash-2026-07-30 держит прежнюю цепочку, и
  теги не пушатся сами — нужен --follow-tags, иначе 261 ссылка на хеши в AUDIT.md умрет.
- Что значит черный список для чтения: GameIntentionResolver.Read и GameAvailable отвечают по-разному.
- Уникальные индексы на Subscriptions и на живой портрет персонажа — правка InitialCreate, то есть ресет базы.
...
высоких и сопоставимая среди средних, так что непроверенную строку нельзя брать в работу как факт.

Второй гейт, двоичный: ярус e2e сейчас красный — 150 прошло, 78 упало, 13 пропущено, exit 1.
Спеки написаны под формы, которых больше нет; это отдельный корпус работы.
...
Числа в этой таблице — те, что назвали срезы. Опровержение прошло позже и сдвинуло их:
девять средних поднялись до высоких, около сотни средних опустились до низких. Итог после
коррекции — в «Прогрессе» выше, и он же в индексе. Эта таблица оставлена как есть: она
объясняет выставленные оценки, а оценки выставлялись по тому, что видел срез.
...
пытался сломать, и брать ее в работу нельзя, пока не проверена.

| Статус | Severity | id | Находка | Срез | Вердикт | Чем закрыто |
|--------|----------|----|---------|------|---------|-------------|
| Открыто | КРИТИЧНО | `character-retirement-500` | вывод персонажа из игры отвечает 500 для всех | Предметная модель и продуктовая логика | частично неверно |  |
| Открыто | КРИТИЧНО | `devops-api-port-bypasses-perimeter` | API preview-стенда опубликован в интернет в обход basic auth | Сборка, CI/CD, деплой, эксплуатация | подтверждено |  |
| Открыто | КРИТИЧНО | `devops-dependency-scan-cannot-run` | гейт уязвимостей падает не по делу, и вместе с ним умирает вся публикация образов | Сборка, CI/CD, деплой, эксплуатация | подтверждено |  |
| Открыто | КРИТИЧНО | `devops-minio-backups-never-run` | бэкап объектного хранилища не выполняется под cron, проверка его не покрывает, а восстановление задокументировано нерабочими командами | Сборка, CI/CD, деплой, эксплуатация | подтверждено |  |
| Открыто | КРИТИЧНО | `razor-templates-dead-body-is-json` | механизм шаблонов писем мертв, телом каждого письма аккаунта уходит JSON | Гигиена кода и техдолг | подтверждено |  |
| Открыто | ВЫСОКАЯ | `403-reason-discarded` | причина отказа, которую сервер называет в 68 местах, заменяется одной общей фразой | Пользовательские сценарии | подтверждено |  |
| Открыто | ВЫСОКАЯ | `accent-fill-invisible-label` | заливка `$accent-*` с текстом `$text-on-*` дает контраст 1.00-1.15 | Качество фронтенда: дизайн-система, a11y, производительность | подтверждено |  |
| Открыто | ВЫСОКАЯ | `agent-model-pins-contradict-a-direct-order` | три агента запинены на sonnet вопреки прямому указанию | Процесс разработки: человек и Claude | подтверждено |  |
| Открыто | ВЫСОКАЯ | `api-design-sort-contract` | раздел Sorting описывает несуществующий формат, противоречащий соседнему разделу того же документа | Документация | подтверждено |  |
| Открыто | ВЫСОКАЯ | `api-english-error-titles` | 255 английских сообщений домена выходят в русский интерфейс | Копирайт и язык интерфейса | подтверждено |  |
| Открыто | ВЫСОКАЯ | `api-idempotency-key-blocked-by-cors` | заголовок идемпотентности не в CORS-разрешении, загрузка файлов не проходит preflight в cross-origin конфигурации | Дизайн HTTP API | подтверждено |  |
| Открыто | ВЫСОКАЯ | `api-rate-limit-partition-blind` | партиционирование по аккаунту не работает никогда: лимитер стоит до аутентификации | Дизайн HTTP API | подтверждено |  |
| Открыто | ВЫСОКАЯ | `api-secret-in-path` | одноразовый токен восстановления доступа едет в пути URL, вопреки собственному правилу проекта | Дизайн HTTP API | подтверждено |  |
| Открыто | ВЫСОКАЯ | `api-user-schema-three-fidelities` | одна схема User отдается в трех разных полнотах, и различить их по ответу нельзя | Дизайн HTTP API | подтверждено |  |
| Открыто | ВЫСОКАЯ | `attribute-schema-hard-delete` | жесткое удаление документа Mongo, на который ссылается строка Postgres | Полиглотное хранение (PG / Mongo / MinIO) | подтверждено |  |
| Открыто | ВЫСОКАЯ | `award-tier-5-uneditable` | устаревший док про Tier прикрывает нередактируемую строку каталога | Комментарии: правда, польза, объем | подтверждено |  |
| Открыто | ВЫСОКАЯ | `ban-not-checked-on-edit-and-reviews` | обычный бан обходится правкой и отзывами | Предметная модель и продуктовая логика | подтверждено |  |
| Открыто | ВЫСОКАЯ | `button-variant-contrast` | белая подпись основной и опасной кнопки не проходит AA | Качество фронтенда: дизайн-система, a11y, производительность | подтверждено |  |
| Открыто | ВЫСОКАЯ | `captive-dbcontext-in-connection-singleton` | один DmDbContext на все подключения SignalR, разделяемый и никогда не освобождаемый | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | подтверждено, severity завышен |  |
| Открыто | ВЫСОКАЯ | `character-attributes-uniqueness-missing` | дубль атрибута делает персонажа нередактируемым навсегда | Реляционное хранилище и EF Core | подтверждено |  |
| Открыто | ВЫСОКАЯ | `chat-room-two-authorization-models` | чат игровой комнаты отвечает 404 всем, потому что API-слой и домен авторизуют его по разным моделям | Архитектура бэкенда и границы модулей | подтверждено |  |
| Открыто | ВЫСОКАЯ | `comment-delete-fake-success` | комментарий и сообщение помечаются удаленными независимо от ответа сервера | Пользовательские сценарии | подтверждено |  |
| Открыто | ВЫСОКАЯ | `comments-fk-hand-edit-unguarded` | регенерация миграции ломает комментарии, и заявленного стража не существует | Реляционное хранилище и EF Core | подтверждено |  |
| Открыто | ВЫСОКАЯ | `contract-mirror-allowlist` | сшивка контракта покрывает 13 интерфейсов из 246 | Стратегия и качество тестов | severity-corrected |  |
| Открыто | ВЫСОКАЯ | `create-game-tags-discarded` | выбранные теги игры молча выбрасываются | Пользовательские сценарии | подтверждено |  |
| Открыто | ВЫСОКАЯ | `deactivated-user-frees-its-email` | деактивация освобождает почту и логин на словах, но не в схеме | Реляционное хранилище и EF Core | подтверждено |  |
| Открыто | ВЫСОКАЯ | `detail-stores-race` | gameDetails и blogDetails коммитят ответы без проверки актуальности | Архитектура фронтенда (FSD) | подтверждено |  |
| Открыто | ВЫСОКАЯ | `devops-forwarded-headers-never-trusted` | за nginx все запросы приходят с одного адреса | Сборка, CI/CD, деплой, эксплуатация | подтверждено |  |
| Открыто | ВЫСОКАЯ | `devops-htpasswd-committed` | хеш пароля preview-стенда лежит в репозитории, а .gitignore создает видимость обратного | Сборка, CI/CD, деплой, эксплуатация | severity-corrected |  |
| Открыто | ВЫСОКАЯ | `devops-imgproxy-unreachable-in-deployment` | в развернутой топологии не отдаются аватары и превью | Сборка, CI/CD, деплой, эксплуатация | подтверждено |  |
| Открыто | ВЫСОКАЯ | `devops-installer-cannot-finish` | документированная установка на сервер обрывается на последнем шаге | Сборка, CI/CD, деплой, эксплуатация | подтверждено |  |
| Открыто | ВЫСОКАЯ | `devops-nothing-consumes-published-images` | доставка описана как pull, выполнена как build, включена нигде | Сборка, CI/CD, деплой, эксплуатация | подтверждено |  |
| Открыто | ВЫСОКАЯ | `devops-secure-cookie-over-plain-http` | на развернутом стенде нельзя войти | Сборка, CI/CD, деплой, эксплуатация | подтверждено |  |
| Открыто | ВЫСОКАЯ | `draft-visibility-ignored-for-games` | публичный черновик игры в списке есть, по ссылке 403 | Предметная модель и продуктовая логика | подтверждено |  |
| Открыто | ВЫСОКАЯ | `e2e-green-by-skip` | 14 мест, где падение превращается в пропуск, и одно из них прячет живую поломку | Стратегия и качество тестов | частично неверно |  |
| Открыто | ВЫСОКАЯ | `e2e-login-as-nonexistent-account` | спека логина входит под аккаунтом, которого нет в сидере | Стратегия и качество тестов | подтверждено |  |
| Открыто | ВЫСОКАЯ | `false-comma-separated-authors` | четыре контроллера документируют формат параметра, которого биндинг не понимает | Комментарии: правда, польза, объем | подтверждено |  |
| Открыто | ВЫСОКАЯ | `form-label-dangling-for` | 21 из 30 меток формы указывают на несуществующий id | Качество фронтенда: дизайн-система, a11y, производительность | подтверждено |  |
| Открыто | ВЫСОКАЯ | `game-activated-emits-wrong-event-type` | уведомление об активации игры помечено как "новая игра" | Гигиена кода и техдолг | подтверждено |  |
| Открыто | ВЫСОКАЯ | `game-application-review-unreachable` | заявку в игру нельзя ни принять, ни отклонить, если в игре нет NPC | Пользовательские сценарии | подтверждено, severity завышен |  |
| Открыто | ВЫСОКАЯ | `git-guard-blocks-two-of-ten` | хук против потери работы пропускает восемь эквивалентных команд | Процесс разработки: человек и Claude | подтверждено |  |
| Открыто | ВЫСОКАЯ | `health-detail-public` | `/_health/detail` с текстами исключений и `/metrics` открыты в интернет вместе с портом API | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | подтверждено |  |
| Открыто | ВЫСОКАЯ | `hub-singleton-captures-scoped-dbcontext` | аутентификация хаба через захваченный DbContext | Безопасность | severity-corrected |  |
| Открыто | ВЫСОКАЯ | `icon-buttons-unnamed` | восемь иконочных кнопок без доступного имени, и расхождение двух чатов | Качество фронтенда: дизайн-система, a11y, производительность | подтверждено |  |
| Открыто | ВЫСОКАЯ | `ilike-as-equality` | `ILIKE` вместо равенства: подстановочные знаки в почте и логине и мимо индекса | Реляционное хранилище и EF Core | подтверждено |  |
| Открыто | ВЫСОКАЯ | `ledger-stale-where-compaction-hurts` | реестр требований показывает "не сделано" для сделанного | Процесс разработки: человек и Claude | подтверждено |  |
| Открыто | ВЫСОКАЯ | `local-gate-is-not-the-ci-gate` | pre-push утверждает, что гоняет то же, что CI | Процесс разработки: человек и Claude | severity-corrected |  |
| Открыто | ВЫСОКАЯ | `login-403-silent-success` | вход и регистрация докладывают об успехе на любом ответе, кроме 400 | Пользовательские сценарии | подтверждено |  |
| Открыто | ВЫСОКАЯ | `mentor-cannot-read-curated-game` | ментор не может открыть игру, которую проверяет | Предметная модель и продуктовая логика | подтверждено |  |
| Открыто | ВЫСОКАЯ | `messenger-send-loses-text` | сбой отправки личного сообщения уничтожает набранный текст и черновик | Пользовательские сценарии | подтверждено |  |
| Открыто | ВЫСОКАЯ | `minio-root-credentials` | приложение и imgproxy ходят в объектное хранилище рутом | Полиглотное хранение (PG / Mongo / MinIO) | подтверждено |  |
| Открыто | ВЫСОКАЯ | `moderation-notifications-unreachable` | пять генераторов не могут сработать никогда | Асинхронный обмен сообщениями и realtime | подтверждено |  |
| Открыто | ВЫСОКАЯ | `monitoring-alerting-not-wired` | документ обещает уведомления по алертам, которых некому отправлять | Документация | подтверждено |  |
| Открыто | ВЫСОКАЯ | `mouse-only-controls` | два интерактивных элемента доступны только мышью | Качество фронтенда: дизайн-система, a11y, производительность | частично неверно |  |
| Открыто | ВЫСОКАЯ | `new-message-event-never-bound` | счетчик непрочитанных в мессенджере не обновляется в реальном времени | Асинхронный обмен сообщениями и realtime | подтверждено |  |
| Открыто | ВЫСОКАЯ | `no-dlx-for-notifications` | отравленное событие уведомления исчезает без следа | Асинхронный обмен сообщениями и realtime | подтверждено |  |
| Открыто | ВЫСОКАЯ | `non-persistent-publishing` | рестарт брокера уносит очередь писем и dead-letter очередь | Асинхронный обмен сообщениями и realtime | подтверждено |  |
| Открыто | ВЫСОКАЯ | `notification-labels-three-copies` | подписи событий существуют в трех копиях и уже разошлись | Копирайт и язык интерфейса | подтверждено |  |
| Открыто | ВЫСОКАЯ | `notification-metadata-english-keys` | письма и боты печатают английские имена полей | Копирайт и язык интерфейса | подтверждено |  |
| Открыто | ВЫСОКАЯ | `notification-processor-not-idempotent` | повтор обработки дублирует уведомления, письма и пуши | Асинхронный обмен сообщениями и realtime | подтверждено |  |
| Открыто | ВЫСОКАЯ | `notification-tables-out-of-sync` | три таблицы, которые обязаны совпадать, расходятся на 20 записей | Гигиена кода и техдолг | частично неверно |  |
| Открыто | ВЫСОКАЯ | `placeholder-substitution-into-markup` | плейсхолдер из текста пользователя подставляется как HTML | Безопасность | severity-corrected |  |
| Открыто | ВЫСОКАЯ | `preview-api-port-published` | basic auth обходится через порт 5000 | Безопасность | подтверждено |  |
| Открыто | ВЫСОКАЯ | `private-addressee-snapshot-never-written` | правило адресата не работает никогда | Безопасность | подтверждено |  |
| Открыто | ВЫСОКАЯ | `proxy-headers-not-configured-with-nginx` | за прокси все клиенты сливаются в один адрес | Безопасность | подтверждено |  |
| Открыто | ВЫСОКАЯ | `publication-comments-skip-blacklist` | четвертая копия GetDiscussion потеряла фильтр черного списка | Гигиена кода и техдолг | подтверждено |  |
| Открыто | ВЫСОКАЯ | `room-access-policy-not-enforced` | ReadOnly-доступ к комнате разрешает писать | Предметная модель и продуктовая логика | подтверждено |  |
| Открыто | ВЫСОКАЯ | `room-number-never-allocated` | колонка для URL комнаты не заполняется ни одним путем записи | Реляционное хранилище и EF Core | severity-corrected |  |
| Открыто | ВЫСОКАЯ | `session-truth-split` | при истечении сессии store остается авторизованным | Архитектура фронтенда (FSD) | подтверждено |  |
| Открыто | ВЫСОКАЯ | `signout-reports-success-on-failure` | «Выйти» и «Выйти со всех устройств» гасят локальную сессию независимо от сервера | Пользовательские сценарии | подтверждено |  |
| Открыто | ВЫСОКАЯ | `silent-mutations-in-forum-and-room` | четыре модераторских и мастерских действия не сообщают ни об успехе, ни об отказе | Пользовательские сценарии | severity-corrected |  |
| Открыто | ВЫСОКАЯ | `smtp-connection-per-letter` | на каждое письмо открывается новая SMTP-сессия, которая не закрывается | Асинхронный обмен сообщениями и realtime | подтверждено |  |
| Открыто | ВЫСОКАЯ | `smtp-session-per-message` | на каждое письмо открывается SMTP-сессия, которую никто не закрывает | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | подтверждено |  |
| Открыто | ВЫСОКАЯ | `tabs-arrow-navigation-dead-end` | стрелкой вправо нельзя дойти дальше второго таба | Качество фронтенда: дизайн-система, a11y, производительность | подтверждено |  |
| Открыто | ВЫСОКАЯ | `two-contracts-one-notification` | одно уведомление приезжает клиенту в двух разных форматах | Асинхронный обмен сообщениями и realtime | severity-corrected |  |
| Открыто | ВЫСОКАЯ | `unescaped-metadata-in-outgoing-messages` | метаданные подставляются в HTML письма и Telegram без экранирования | Асинхронный обмен сообщениями и realtime | подтверждено |  |
| Открыто | ВЫСОКАЯ | `unread-counter-key-not-unique` | код считает тройку ключом, а база про это не знает | Полиглотное хранение (PG / Mongo / MinIO) | severity-corrected |  |
| Открыто | СРЕДНЯЯ | `activation-retry-field-mismatch` | защита от повторной активации мертва из-за расхождения имени поля | Пользовательские сценарии | не проверялась |  |
| Открыто | СРЕДНЯЯ | `activitypolicy-overpromise` | комментарий обещает единственную точку правки, которой нет через границу стека | Комментарии: правда, польза, объем | confirmed |  |
| Открыто | СРЕДНЯЯ | `alerts-have-no-delivery` | восемь правил вычисляются и никуда не отправляются | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | confirmed |  |
| Открыто | СРЕДНЯЯ | `api-design-ratelimits-incomplete` | таблица лимитов перечисляет 4 политики из 7 | Документация | confirmed |  |
| Открыто | СРЕДНЯЯ | `api-envelope-split` | 70 из 278 ответов с телом отдают DTO без конверта, а страхующий тест покрывает три эндпоинта | Дизайн HTTP API | confirmed |  |
| Открыто | СРЕДНЯЯ | `api-error-media-type-fiction` | опубликованный контракт называет неверный media type для всех 890 ответов с ошибкой | Дизайн HTTP API | не проверялась |  |
| Открыто | СРЕДНЯЯ | `api-guid-only-reviews-subroute` | один подмаршрут игры отказывается от публичного идентификатора | Дизайн HTTP API | confirmed |  |
| Открыто | СРЕДНЯЯ | `api-query-vocabulary-drift` | один и тот же фильтр называется по-разному на соседних эндпоинтах | Дизайн HTTP API | confirmed |  |
| Открыто | СРЕДНЯЯ | `api-resource-background-leak` | после гонки фонового обновления с принудительным ресурс больше не обновляется | Архитектура фронтенда (FSD) | confirmed |  |
| Открыто | СРЕДНЯЯ | `api-response-dto-as-request-body` | 9 из 44 `PATCH`/`PUT` принимают в тело DTO ответа | Дизайн HTTP API | confirmed |  |
| Открыто | СРЕДНЯЯ | `api-security-log-limit-one-sided` | ограничение размера страницы обрезано только сверху, и это четвертая идиома для одного правила | Дизайн HTTP API | confirmed |  |
| Открыто | СРЕДНЯЯ | `api-services-reach-past-the-domain` | шесть API-сервисов держат доменные репозитории, два авторизуют сами | Архитектура бэкенда и границы модулей | partly-wrong |  |
| Открыто | СРЕДНЯЯ | `api-sort-contract-fiction` | документированный формат сортировки не принимает ни один эндпоинт, а неизвестное значение молча игнорируется | Дизайн HTTP API | не проверялась |  |
| Открыто | СРЕДНЯЯ | `api-status-two-doors` | статус игры меняется двумя эндпоинтами с разными правилами, и один из них отвечает 200 на отказ | Дизайн HTTP API | не проверялась |  |
| Открыто | СРЕДНЯЯ | `api-unbounded-collections` | 47 коллекций возвращаются целиком, часть из них растет без предела, а одна молча урезана до 50 | Дизайн HTTP API | не проверялась |  |
| Открыто | СРЕДНЯЯ | `arch-test-guards-only-postgres` | правило про слой данных названо через один тип и потому проверяет одно хранилище из трех | Полиглотное хранение (PG / Mongo / MinIO) | confirmed |  |
| Открыто | СРЕДНЯЯ | `arch-tests-cover-the-narrowest-slice` | три правила из десятка проверяемых утверждений PATTERNS, и воркеры с сидером вне зоны видимости | Архитектура бэкенда и границы модулей | confirmed |  |
| Открыто | СРЕДНЯЯ | `audit-log-half-empty` | пять из одиннадцати типов событий не пишет никто | Безопасность | confirmed |  |
| Открыто | СРЕДНЯЯ | `audit-trail-on-one-unpushed-tag` | 106 из 110 ссылок реестра держит один локальный тег | Процесс разработки: человек и Claude | не проверялась |  |
| Открыто | СРЕДНЯЯ | `auth-guard-drops-destination` | гостя и пользователя с истекшей сессией уводит на главную, теряя, куда он шел | Пользовательские сценарии | confirmed |  |
| Открыто | СРЕДНЯЯ | `auto-subscribe-wrong-user` | автоподписка при выходе персонажа оформляется на действующего, а не на игрока | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | СРЕДНЯЯ | `barrel-rule-not-enforced` | линтер не проверяет запрет deep-import, хотя документ и конфиг это утверждают | Архитектура фронтенда (FSD) | не проверялась |  |
| Открыто | СРЕДНЯЯ | `blog-status-five-names` | статус блога называется двумя словами на одном экране | Копирайт и язык интерфейса | не проверялась |  |
| Открыто | СРЕДНЯЯ | `chats-list-error-as-empty` | неудачная загрузка переписок показана как «Нет переписок» | Пользовательские сценарии | не проверялась |  |
| Открыто | СРЕДНЯЯ | `claude-readme-antipatterns` | 509 строк, на которые никто не ссылается, учат тому, что проект запрещает | Документация | не проверялась |  |
| Открыто | СРЕДНЯЯ | `connection-map-race` | гонка connect/disconnect теряет соединение из карты пользователей | Асинхронный обмен сообщениями и realtime | confirmed |  |
| Открыто | СРЕДНЯЯ | `contract-test-coverage` | контрактный тест держит 13 DTO из 335 опубликованных схем | Архитектура фронтенда (FSD) | confirmed |  |
| Открыто | СРЕДНЯЯ | `cross-store-write-unrepaired` | вторая запись в другое хранилище не атомарна и ничем не ремонтируется | Полиглотное хранение (PG / Mongo / MinIO) | не проверялась |  |
| Открыто | СРЕДНЯЯ | `csp-unsafe-inline-forced-by-renderer` | 'unsafe-inline' в script-src держится на косметике | Безопасность | confirmed |  |
| Открыто | СРЕДНЯЯ | `deployment-image-names` | имена публикуемых образов в документе не те, что публикует CI | Документация | не проверялась |  |
| Открыто | СРЕДНЯЯ | `devops-compose-topology-step-can-pass-vacuously` | сравнение инсталлятора и юнита проходит при пустом результате | Сборка, CI/CD, деплой, эксплуатация | confirmed |  |
| Открыто | СРЕДНЯЯ | `devops-env-example-ships-working-secrets` | все секреты кроме одного имеют рабочее значение в репозитории | Сборка, CI/CD, деплой, эксплуатация | не проверялась |  |
| Открыто | СРЕДНЯЯ | `devops-mirror-command-starts-everything` | документированная команда зеркала поднимает второй экземпляр всего, включая миграции против main | Сборка, CI/CD, деплой, эксплуатация | не проверялась |  |
| Открыто | СРЕДНЯЯ | `devops-nginx-body-limit-vs-api-limit` | загрузка больше 1 МБ обрывается на краю | Сборка, CI/CD, деплой, эксплуатация | confirmed |  |
| Открыто | СРЕДНЯЯ | `devops-no-alertmanager` | восемь правил алертинга срабатывают в никуда | Сборка, CI/CD, деплой, эксплуатация | не проверялась |  |
| Открыто | СРЕДНЯЯ | `devops-no-config-validation-gates` | конфигурацию края никто не проверяет, и развернутая топология ни разу не запускается | Сборка, CI/CD, деплой, эксплуатация | confirmed |  |
| Открыто | СРЕДНЯЯ | `devops-no-dotnet-coverage-gate` | покрытие бэкенда собирается и не проверяется | Сборка, CI/CD, деплой, эксплуатация | confirmed |  |
| Открыто | СРЕДНЯЯ | `devops-no-log-rotation` | журналы контейнеров растут без ограничения | Сборка, CI/CD, деплой, эксплуатация | confirmed |  |
| Открыто | СРЕДНЯЯ | `dialog-tier-fragmentation` | четыре реализации модального окна, и Escape перестает работать после клика по диалогу | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | СРЕДНЯЯ | `dm2-migration-arithmetic` | план переименований не сходится сам с собой, а предписанный BBCode рендерится в текст | Документация | severity-corrected |  |
| Открыто | СРЕДНЯЯ | `dmsh-cannot-start-the-stack` | документированный старт на Linux падает на первой команде | Гигиена кода и техдолг | не проверялась |  |
| Открыто | СРЕДНЯЯ | `domain-purity-has-no-guard` | правило, нарушавшееся трижды, не проверяется ничем | Процесс разработки: человек и Claude | confirmed |  |
| Открыто | СРЕДНЯЯ | `double-notify-on-status-change` | подписчик игры получает два уведомления об одном событии | Асинхронный обмен сообщениями и realtime | confirmed |  |
| Открыто | СРЕДНЯЯ | `e2e-assertions-behind-dead-selectors` | тесты сценария персонажей ничего не проверяют | Пользовательские сценарии | partly-wrong |  |
| Открыто | СРЕДНЯЯ | `e2e-tests-without-assertions` | 11 тестов не могут провалиться | Стратегия и качество тестов | confirmed |  |
| Открыто | СРЕДНЯЯ | `email-vs-pochta` | поле называется "Почта", а ошибка под ним говорит "Email" | Копирайт и язык интерфейса | confirmed |  |
| Открыто | СРЕДНЯЯ | `envelope-invariant-guarded-by-allowlist` | инвариант конверта проверяется на трех URL из семидесяти пяти | Стратегия и качество тестов | partly-wrong |  |
| Открыто | СРЕДНЯЯ | `error-status-dropped` | store сущностей теряют HTTP-статус, и одинаковый отказ выглядит по-разному | Архитектура фронтенда (FSD) | confirmed |  |
| Открыто | СРЕДНЯЯ | `fail-open-catches` | модерационные проверки при ошибке отвечают «нарушений нет» | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | СРЕДНЯЯ | `false-fk-test-coverage` | комментарий в миграции утверждает, что отсутствие FK защищено тестом; тест его не проверяет | Комментарии: правда, польза, объем | не проверялась |  |
| Открыто | СРЕДНЯЯ | `fifth-button-variant-duplicated` | «контурная пилюля» скопирована в четырех файлах вне системы кнопок | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | СРЕДНЯЯ | `filter-codec-x8` | восемь копий кодека фильтра, и они уже разошлись | Архитектура фронтенда (FSD) | confirmed |  |
| Открыто | СРЕДНЯЯ | `fixes-without-a-failing-test` | 30 из 74 поведенческих правок не закреплены ничем | Процесс разработки: человек и Claude | partly-wrong |  |
| Открыто | СРЕДНЯЯ | `flip-ignores-reduced-motion` | единственная JS-анимация не смотрит на настройку системы | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | СРЕДНЯЯ | `fonts-ttf-no-display` | 1 МБ несжатых шрифтов, без `font-display` и без preload | Качество фронтенда: дизайн-система, a11y, производительность | не проверялась |  |
| Открыто | СРЕДНЯЯ | `formfield-error-visual-dead` | визуальная индикация ошибки поля не срабатывает никогда | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | СРЕДНЯЯ | `frontend-agent-points-at-the-owners-port` | бриф агента ведет на :5173 | Процесс разработки: человек и Claude | confirmed |  |
| Открыто | СРЕДНЯЯ | `game-blog-publicid-not-unique-not-indexed` | читаемый идентификатор игры и блога без уникальности и без индекса | Реляционное хранилище и EF Core | не проверялась |  |
| Открыто | СРЕДНЯЯ | `game-intention-authorization-untested` | 9 из 19 ветвей самого крупного резолвера прав не проверяет ни один тест | Стратегия и качество тестов | не проверялась |  |
| Открыто | СРЕДНЯЯ | `game-page-error-is-one-sentence` | удаленная, приватная и недоступная игра выглядят одинаково | Пользовательские сценарии | confirmed |  |
| Открыто | СРЕДНЯЯ | `game-role-api-vocabulary-mismatch` | ментор в API отдается как "unknown", три фильтра мертвы | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | СРЕДНЯЯ | `gametags-uniqueness-missing` | дубль тега ломает фильтр обязательных тегов | Реляционное хранилище и EF Core | confirmed |  |
| Открыто | СРЕДНЯЯ | `gates-report-but-do-not-enforce` | что CI собирает и не проверяет | Стратегия и качество тестов | confirmed |  |
| Открыто | СРЕДНЯЯ | `hardcoded-plurals` | счетчики с зашитой формой существительного | Копирайт и язык интерфейса | confirmed |  |
| Открыто | СРЕДНЯЯ | `hosted-services-bypass-the-domain` | 9 из 10 фоновых задач HTTP-хоста работают со стором напрямую, три разных подхода в одной папке | Архитектура бэкенда и границы модулей | partly-wrong |  |
| Открыто | СРЕДНЯЯ | `htpasswd-committed` | файл basic-auth preview-стенда лежит в git, хотя .gitignore его перечисляет | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | не проверялась |  |
| Открыто | СРЕДНЯЯ | `icon-name-type-hole` | типизация имен иконок не защищает ни от одной опечатки | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | СРЕДНЯЯ | `input-focus-indicator-too-weak` | фокус на поле ввода обозначен изменением оттенка рамки на 1.8:1 | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | СРЕДНЯЯ | `invitation-game-link-404` | ссылка на игру в приглашении ведет на «Страница не найдена» | Пользовательские сценарии | не проверялась |  |
| Открыто | СРЕДНЯЯ | `lifted-bans-disappear` | снятый бан исчезает из истории, а IsLifted всегда false | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | СРЕДНЯЯ | `likes-uniqueness-missing` | двойной лайк накручивает счетчик и достижение | Реляционное хранилище и EF Core | confirmed |  |
| Открыто | СРЕДНЯЯ | `local-setup-newbie-claim` | обещание "все пользователи начинают с 0 постов" неверно после сида | Документация | confirmed |  |
| Открыто | СРЕДНЯЯ | `lockout-reset-clears-every-address` | истекший lockout одной пары сбрасывает счетчики всех | Безопасность | confirmed |  |
| Открыто | СРЕДНЯЯ | `mentor-vs-nastavnik` | роль наставника на странице настроек игры названа "Ментор" | Копирайт и язык интерфейса | confirmed |  |
| Открыто | СРЕДНЯЯ | `message-search-race` | результаты поиска не соответствуют запросу и сортировке | Архитектура фронтенда (FSD) | не проверялась |  |
| Открыто | СРЕДНЯЯ | `modeled-never-implemented` | четыре понятия существуют только в enum'ах | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | СРЕДНЯЯ | `moderation-gate-copy-missing` | отказ в доступе объяснен на 12 страницах из 21 | Копирайт и язык интерфейса | confirmed |  |
| Открыто | СРЕДНЯЯ | `moderation-silent-catches` | пять голых catch в модерации отвечают "нарушений нет" | Гигиена кода и техдолг | confirmed |  |
| Открыто | СРЕДНЯЯ | `newbie-seven-definitions` | «новичок» определен семь раз и из двух источников | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | СРЕДНЯЯ | `no-abstraction-for-the-object-store` | у третьего хранилища нет ни репозитория, ни единой точки | Полиглотное хранение (PG / Mongo / MinIO) | confirmed |  |
| Открыто | СРЕДНЯЯ | `no-composition-root-test` | контейнер не проверяется ничем, и захват scoped-зависимости singleton-ом обнаружить нечему | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | confirmed |  |
| Открыто | СРЕДНЯЯ | `no-consumer-metrics` | падение доставки писем и уведомлений невидимо для всех дашбордов и алертов | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | не проверялась |  |
| Открыто | СРЕДНЯЯ | `no-dotnet-format-gate` | правило форматирования записано, инструмент падает на 177 файлах, гейта нет | Гигиена кода и техдолг | confirmed |  |
| Открыто | СРЕДНЯЯ | `no-terminology-standard` | нет словаря терминов и нет автопроверки правил | Копирайт и язык интерфейса | confirmed |  |
| Открыто | СРЕДНЯЯ | `no-unsaved-guard` | уход со страницы уносит все набранное там, где нет draft-key | Пользовательские сценарии | confirmed |  |
| Открыто | СРЕДНЯЯ | `notification-routing-test-cannot-fail` | тест, гарантированный компилятором, а не поведением | Стратегия и качество тестов | confirmed |  |
| Открыто | СРЕДНЯЯ | `notification-settings-has-no-domain-layer` | настройки уведомлений живут в HTTP-хосте: контракт, репозиторий над Mongo и правила | Архитектура бэкенда и границы модулей | не проверялась |  |
| Открыто | СРЕДНЯЯ | `pendency-reminder-publishes-before-commit` | сбой SaveChanges приводит к повторной рассылке напоминаний | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | confirmed |  |
| Открыто | СРЕДНЯЯ | `probation-threshold-duplicated` | инвариант между конфигурацией и вычисляемой колонкой держится на предупреждении в логе | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | confirmed |  |
| Открыто | СРЕДНЯЯ | `profile-avatar-cls` | аватар профиля резервирует квадрат под неквадратную картинку | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | СРЕДНЯЯ | `progress-stale-snapshot` | живой документ прогресса датирован и уже неверен в числах и в инвентаре | Документация | confirmed |  |
| Открыто | СРЕДНЯЯ | `public-bucket-for-every-upload-type` | приватные вложения читаются анонимно, удаленные читаются после удаления | Полиглотное хранение (PG / Mongo / MinIO) | не проверялась |  |
...
| Открыто | СРЕДНЯЯ | `seed-board-counter-inconsistent` | свежая база показывает у борда неверное число тем | Реляционное хранилище и EF Core | confirmed |  |
| Открыто | СРЕДНЯЯ | `silent-authorization-drop` | запрещенное изменение возвращает 200 OK и ничего не делает | Предметная модель и продуктовая логика | не проверялась |  |
| Открыто | СРЕДНЯЯ | `silent-drop-is-invisible` | событие без привязки исчезает, и это ничем не измеряется | Асинхронный обмен сообщениями и realtime | partly-wrong |  |
| Открыто | СРЕДНЯЯ | `stale-award-catalog-count` | "6 rows" в шести файлах кода и в PROGRESS.md, строк семь | Комментарии: правда, польза, объем | не проверялась |  |
| Открыто | СРЕДНЯЯ | `stale-module-cs-replaced` | Startup утверждает, что Module.cs больше нет, и тут же их регистрирует | Комментарии: правда, польза, объем | не проверялась |  |
| Открыто | СРЕДНЯЯ | `stale-response-404` | публичный OpenAPI обещает 404 там, где endpoint отдает 410 | Комментарии: правда, польза, объем | partly-wrong |  |
| Открыто | СРЕДНЯЯ | `stale-topiccard-consumers` | шапка TopicCard указывает на два файла, один из которых не существует, а второй его не рендерит | Комментарии: правда, польза, объем | confirmed |  |
| Открыто | СРЕДНЯЯ | `subscriptions-uniqueness-missing` | подписка не уникальна в схеме, письма дублируются, отписка не отписывает | Реляционное хранилище и EF Core | не проверялась |  |
| Открыто | СРЕДНЯЯ | `suspicious-login-never-fires` | детектор читает журнал после записи текущего входа | Безопасность | confirmed |  |
| Открыто | СРЕДНЯЯ | `system-md-middleware-order` | SYSTEM.md перечисляет конвейер в порядке, который сам проект называет дефектом | Документация | не проверялась |  |
| Открыто | СРЕДНЯЯ | `testing-no-e2e` | руководство по тестам не знает про целый ярус из 32 файлов, блокирующий публикацию | Документация | confirmed |  |
| Открыто | СРЕДНЯЯ | `tests-assert-nothing` | три из четырех тестов слоя не проверяют поведение | Асинхронный обмен сообщениями и realtime | confirmed |  |
| Открыто | СРЕДНЯЯ | `tooling-gap` | ничто в CI не может поймать ни одну из находок выше | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | СРЕДНЯЯ | `topik-vs-tema` | форумная сущность называется топиком и темой, включая одну и ту же пару полей | Копирайт и язык интерфейса | confirmed |  |
| Открыто | СРЕДНЯЯ | `untested-workers-and-realtime` | 4366 строк воркеров и 319 строк realtime без ни одного теста | Стратегия и качество тестов | confirmed |  |
| Открыто | СРЕДНЯЯ | `url-structure-page-param` | документ URL обещает `?page=N`, которого в проекте нет | Документация | не проверялась |  |
| Открыто | СРЕДНЯЯ | `user-list-runs-profile-enrichment` | 20+ агрегатов на каждую страницу списка пользователей | Реляционное хранилище и EF Core | confirmed |  |
| Открыто | СРЕДНЯЯ | `validation-title-leak` | при отказе валидации в русский интерфейс попадает английское "Validation failed" | Архитектура фронтенда (FSD) | confirmed |  |
| Открыто | СРЕДНЯЯ | `weak-verify-on-patch-entities` | имя теста обещает обновление, утверждение проверяет только факт вызова | Стратегия и качество тестов | confirmed |  |
| Открыто | СРЕДНЯЯ | `webhook-secret-comparison-untested` | тест на неверный секрет не может дойти до сравнения | Стратегия и качество тестов | confirmed |  |
| Открыто | СРЕДНЯЯ | `worker-consumer-blocks-startup` | оба воркера подписываются синхронно внутри StartAsync без страховки и уходят в crash loop | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | не проверялась |  |
| Открыто | НИЗКАЯ | `access-policy-flags-two-jobs` | один [Flags] enum обслуживает две разные роли | Предметная модель и продуктовая логика | partly-wrong |  |
| Открыто | НИЗКАЯ | `api-audience-header-on-every-operation` | рендерящий заголовок объявлен у всех 363 операций | Дизайн HTTP API | confirmed |  |
| Открыто | НИЗКАЯ | `api-design-exceptions-list-stale` | реестр "записанных исключений" перечисляет 4 из как минимум 9 | Документация | severity-corrected |  |
| Открыто | НИЗКАЯ | `api-discussion-orphaned` | форма "обсуждение" реализована в четырех сервисах и выведена одним маршрутом | Дизайн HTTP API | partly-wrong |  |
| Открыто | НИЗКАЯ | `api-doc-audience-wrong-layer` | документ вменяет проверку авторства слою, который ее не делает | Дизайн HTTP API | severity-corrected |  |
| Открыто | НИЗКАЯ | `api-doc-omits-followed-conventions` | документ не описывает три соглашения, которым код следует, и называет класс теста | Дизайн HTTP API | confirmed |  |
| Открыто | НИЗКАЯ | `api-doc-rate-limit-table-stale` | таблица лимитов описывает четыре политики из шести, а самые дорогие чтения не покрыты ни одной | Дизайн HTTP API | severity-corrected |  |
| Открыто | НИЗКАЯ | `api-envelope-metadata-dead` | поле `metadata` есть в 42 схемах контракта, не заполняется ни разу и не имеет типа | Дизайн HTTP API | confirmed |  |
| Открыто | НИЗКАЯ | `api-error-shape-three-producers` | тело ошибки строят три места, два из них в форме, которой нет ни у одного другого ответа | Дизайн HTTP API | confirmed |  |
| Открыто | НИЗКАЯ | `api-identity-comments-and-operationids-lie` | три утверждения о механизмах, которых нет | Дизайн HTTP API | confirmed |  |
| Открыто | НИЗКАЯ | `api-ignored-userid-filter` | опубликованный и задокументированный фильтр `userId` не читается | Дизайн HTTP API | severity-corrected |  |
| Открыто | НИЗКАЯ | `api-location-header-lies` | 62 ответа `201`, ни один не объявляет `Location`, девять ставят его на чужой ресурс | Дизайн HTTP API | partly-wrong |  |
| Открыто | НИЗКАЯ | `api-optional-cannot-clear` | трехзначная семантика `PATCH` не умеет выразить "очистить" | Дизайн HTTP API | confirmed |  |
| Открыто | НИЗКАЯ | `api-response-cache-inert` | кеширующий слой не может дать ни одного попадания, потому что единственный клиент шлет `no-cache` на каждом запросе | Дизайн HTTP API | severity-corrected |  |
| Открыто | НИЗКАЯ | `api-two-paging-shapes` | два DTO пагинации, поле `number` в двух разных значениях, и `paging: null` у 46 списков | Дизайн HTTP API | partly-wrong |  |
| Открыто | НИЗКАЯ | `arch-test-comment-miscounts` | комментарий говорит "пять контроллеров", их шесть | Архитектура бэкенда и границы модулей | confirmed |  |
| Открыто | НИЗКАЯ | `attribute-schema-isremoved-dead` | контракт `IRemovable` объявлен, но не соблюдается ни на записи, ни на чтении | Полиглотное хранение (PG / Mongo / MinIO) | severity-corrected |  |
| Открыто | НИЗКАЯ | `audit-state-memory-misattributes-its-own-numbers` | "опровергнуто оппонентом 33" неверно | Процесс разработки: человек и Claude | partly-wrong |  |
| Открыто | НИЗКАЯ | `auth-store-id-root` | идентификатор store сессии не совпадает ни с файлом, ни с хуком | Архитектура фронтенда (FSD) | confirmed |  |
| Открыто | НИЗКАЯ | `ban-active-predicate-duplicated` | «забанен прямо сейчас» посчитано по-другому | Предметная модель и продуктовая логика | severity-corrected |  |
| Открыто | НИЗКАЯ | `ban-before-password-not-pinned` | комментарий утверждает порядок проверок, утверждение его не проверяет | Стратегия и качество тестов | confirmed |  |
| Открыто | НИЗКАЯ | `blueprint-deviations-in-api-and-infrastructure-layout` | три отступления от собственного чертежа | Архитектура бэкенда и границы модулей | partly-wrong |  |
| Открыто | НИЗКАЯ | `breakpoints-six-values` | шесть конкурирующих значений точки перелома, три имени для одной | Качество фронтенда: дизайн-система, a11y, производительность | severity-corrected |  |
| Открыто | НИЗКАЯ | `bucket-orphans-unreconciled` | объект без строки не найдет никто, и комментарий это отрицает | Полиглотное хранение (PG / Mongo / MinIO) | partly-wrong |  |
| Открыто | НИЗКАЯ | `character-npc-guid-empty` | у NPC автор совпадает с гостем | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | НИЗКАЯ | `characters-empty-flash` | список персонажей во время загрузки утверждает, что персонажей нет | Пользовательские сценарии | severity-corrected |  |
| Открыто | НИЗКАЯ | `claude-readme-is-the-doc-the-purge-missed` | 509 строк инвентаря и выдуманных примеров | Процесс разработки: человек и Claude | confirmed |  |
| Открыто | НИЗКАЯ | `code-noparse-do-not-suppress-img-link` | картинки и ссылки рендерятся внутри code/noparse | Безопасность | confirmed |  |
| Открыто | НИЗКАЯ | `codestyle-unified-names-false` | таблица "унифицированных имен между слоями" не описывает ни один слой | Документация | partly-wrong |  |
| Открыто | НИЗКАЯ | `collect-obsolete-keeps-newest-for-any-target` | правило "самый свежий выживает" верно только для аватаров | Полиглотное хранение (PG / Mongo / MinIO) | partly-wrong |  |
| Открыто | НИЗКАЯ | `comment-delete-no-confirm` | удаление комментария не подтверждается, в отличие от поста и топика | Пользовательские сценарии | confirmed |  |
| Открыто | НИЗКАЯ | `comment-subsystem-quadruplication` | четыре стека комментариев, отличающиеся именами типов | Гигиена кода и техдолг | partly-wrong |  |
| Открыто | НИЗКАЯ | `comments-and-docs-that-lie` | четыре ссылки и объяснения, не соответствующие коду | Гигиена кода и техдолг | partly-wrong |  |
| Открыто | НИЗКАЯ | `comments-that-lie` | четыре комментария противоречат коду | Безопасность; Копирайт и язык интерфейса | partly-wrong |  |
| Открыто | НИЗКАЯ | `contacts-list-key-collisions` | список контактов профиля ключуется по неуникальному полю | Качество фронтенда: дизайн-система, a11y, производительность | partly-wrong |  |
| Открыто | НИЗКАЯ | `contract-snapshot-crlf` | снапшот-тест падает на Windows после чистого клона и переписывает файл, диффа которого не видно | Стратегия и качество тестов | severity-corrected |  |
| Открыто | НИЗКАЯ | `core-config-forces-unneeded-stores` | почтовый воркер обязан объявить Postgres и Mongo, которых не касается | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | severity-corrected |  |
| Открыто | НИЗКАЯ | `correlation-token-does-not-cross-the-queue` | документированный сквозной поиск по токену не может работать | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | confirmed |  |
| Открыто | НИЗКАЯ | `create-many-all-or-nothing` | один конфликтующий индекс срывает утверждение всей коллекции | Полиглотное хранение (PG / Mongo / MinIO) | severity-corrected |  |
| Открыто | НИЗКАЯ | `cross-tab-user-switch` | при смене пользователя без выхода сайдбар показывает данные предыдущего | Архитектура фронтенда (FSD) | partly-wrong |  |
| Открыто | НИЗКАЯ | `cypress-remnants` | конфигурация для тестового раннера, которого в проекте нет | Гигиена кода и техдолг | confirmed |  |
| Открыто | НИЗКАЯ | `dangling-internals-visible-to` | InternalsVisibleTo выданы сборкам, которых нет, и продублированы | Архитектура бэкенда и границы модулей | confirmed |  |
| Открыто | НИЗКАЯ | `data-storage-doc-knows-two-stores` | SSOT по хранилищам не знает про третье хранилище | Полиглотное хранение (PG / Mongo / MinIO) | partly-wrong |  |
| Открыто | НИЗКАЯ | `dead-board-update-comment` | комментарий описывает действие над пустым блоком | Комментарии: правда, польза, объем | confirmed |  |
| Открыто | НИЗКАЯ | `dead-config-keys` | четыре ключа конфигурации, которые никто не читает, и два источника, которые никто не монтирует | Гигиена кода и техдолг | severity-corrected |  |
| Открыто | НИЗКАЯ | `dead-description-attributes` | 43 русские подписи на сервере, которых никто не читает | Копирайт и язык интерфейса | partly-wrong |  |
| Открыто | НИЗКАЯ | `dead-kernel-abstraction-with-ambiguous-registration` | IContentBlacklistService не имеет ни одного потребителя и две реализации, которые выбираются порядком регистрации | Архитектура бэкенда и границы модулей | partly-wrong |  |
| Открыто | НИЗКАЯ | `dead-permission-bucket-cache` | описанный в документах кеш рендера не подключен | Безопасность | confirmed |  |
| Открыто | НИЗКАЯ | `dead-reset-all` | resetAllGames и resetAllBlogs вызывают только их собственные тесты | Архитектура фронтенда (FSD) | confirmed |  |
| Открыто | НИЗКАЯ | `dead-test-auth-scaffolding` | сотня строк неработающей подмены аутентификации и комментарий, который врет | Стратегия и качество тестов; Гигиена кода и техдолг | severity-corrected |  |
| Открыто | НИЗКАЯ | `dead-type-forwarders-with-a-false-comment` | файл переадресации типов не нужен никому, а комментарий утверждает обратное | Архитектура бэкенда и границы модулей | confirmed |  |
| Открыто | НИЗКАЯ | `dead-types-and-members` | 14 типов и несколько членов без единой ссылки | Гигиена кода и техдолг | severity-corrected |  |
| Открыто | НИЗКАЯ | `dead-update-builder` | 383 строки инфраструктуры записи без единого потребителя | Реляционное хранилище и EF Core | severity-corrected |  |
| Открыто | НИЗКАЯ | `dead-weight-and-lying-comments` | неиспользуемые ассеты и два комментария не про код | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | НИЗКАЯ | `default-avatar-follows-os-theme` | дефолтный аватар не слушает тему приложения | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | НИЗКАЯ | `democratic-ban-described-as-read-only` | два дока противоречат определению | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | НИЗКАЯ | `deprecated-interface-is-the-only-one-used` | помеченный к удалению интерфейс остается единственным API для двух hosted-сервисов | Асинхронный обмен сообщениями и realtime | confirmed |  |
| Открыто | НИЗКАЯ | `design-language-drift` | эмодзи в интерфейсе и запрещенный разделитель | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | НИЗКАЯ | `devops-backups-volume-unused` | том backups смонтирован в Postgres и никем не используется | Сборка, CI/CD, деплой, эксплуатация | confirmed |  |
| Открыто | НИЗКАЯ | `devops-dm-sh-vs-ps1-drift` | первый запуск на Linux и Mac падает, хотя README обещает те же команды | Сборка, CI/CD, деплой, эксплуатация | severity-corrected |  |
| Открыто | НИЗКАЯ | `devops-docs-numbers-do-not-match-reality` | цифры и пороги в эксплуатационной документации расходятся с файлами | Сборка, CI/CD, деплой, эксплуатация | confirmed |  |
| Открыто | НИЗКАЯ | `devops-external-heartbeat-monitors-the-spa` | рекомендованная внешняя проверка не видит API | Сборка, CI/CD, деплой, эксплуатация | partly-wrong |  |
| Открыто | НИЗКАЯ | `devops-front-image-context-and-build-details` | мелочи сборки образов | Сборка, CI/CD, деплой, эксплуатация | confirmed |  |
| Открыто | НИЗКАЯ | `devops-installer-prints-password-and-trusts-curl-bash` | установщик печатает пароль и ставится из ветки dev | Сборка, CI/CD, деплой, эксплуатация | partly-wrong |  |
| Открыто | НИЗКАЯ | `devops-pre-push-claims-parity-it-does-not-have` | локальный гейт заявляет паритет с CI и не имеет его | Сборка, CI/CD, деплой, эксплуатация | severity-corrected |  |
| Открыто | НИЗКАЯ | `devops-production-config-never-validated` | все проверки гоняются в Development | Сборка, CI/CD, деплой, эксплуатация | partly-wrong |  |
| Открыто | НИЗКАЯ | `devops-unpinned-images` | четыре образа из тринадцати не зафиксированы, и самый опасный из них имеет доступ к docker.sock | Сборка, CI/CD, деплой, эксплуатация | partly-wrong |  |
| Открыто | НИЗКАЯ | `direct-utcnow-in-repositories` | 18 обращений к системным часам мимо `IDateTimeProvider` | Реляционное хранилище и EF Core | confirmed |  |
| Открыто | НИЗКАЯ | `doc-pointers-drift` | указатели "Файл:" ведут не туда, где лежит описанное | Документация | confirmed |  |
| Открыто | НИЗКАЯ | `docs-and-comments-that-lie` | адрес хаба и схема шины в документации не соответствуют коду | Асинхронный обмен сообщениями и realtime | confirmed |  |
| Открыто | НИЗКАЯ | `docstring-promises-a-pattern-guard` | `CallerMistakesShould` охраняет три URL, а обещает шаблон | Стратегия и качество тестов | confirmed |  |
| Открыто | НИЗКАЯ | `docx-not-in-index` | полный план функционала достижим только из первой строки другого документа | Документация | confirmed |  |
| Открыто | НИЗКАЯ | `domain-core-is-not-the-kernel-the-doc-describes` | в ядре есть реализации и правило предметной области, и у него нет тестового проекта | Архитектура бэкенда и границы модулей | partly-wrong |  |
| Открыто | НИЗКАЯ | `doors-reexport-shared` | две из трех дверей entities/user выдают символы, которые лежат в shared | Архитектура фронтенда (FSD) | severity-corrected |  |
| Открыто | НИЗКАЯ | `duplicated-keyframes-in-bundle` | 20 копий одной анимации в собранном CSS | Качество фронтенда: дизайн-система, a11y, производительность | confirmed |  |
| Открыто | НИЗКАЯ | `empty-value-placeholders` | пять разных заглушек для отсутствующего значения, две из них в одной таблице | Копирайт и язык интерфейса | partly-wrong |  |
| Открыто | НИЗКАЯ | `english-fallback-in-tag-selector` | в русском интерфейсе показывается «Failed to load tags» | Пользовательские сценарии | confirmed |  |
| Открыто | НИЗКАЯ | `entity-marker-exemption-by-simple-name` | список исключений сматчен по короткому имени | Стратегия и качество тестов | confirmed |  |
| Открыто | НИЗКАЯ | `enum-layer-conversions-unguarded` | числовой каст между тремя объявлениями SecurityEventType и три плеча `_ =>`, глушащих проверку полноты | Гигиена кода и техдолг | partly-wrong |  |
| Открыто | НИЗКАЯ | `error-handling-ignores-hasstarted` | исключение после начала ответа рвет соединение вторым, незалогированным исключением | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | partly-wrong |  |
| Открыто | НИЗКАЯ | `error-pages-table-columns` | таблица с шестью заголовками и пятью колонками данных читается неверно | Документация | confirmed |  |
| Открыто | НИЗКАЯ | `expression-index-comment-lies` | комментарий отправляет за индексами в миграцию, где их нет | Реляционное хранилище и EF Core | confirmed |  |
| Открыто | НИЗКАЯ | `false-api-standards-doc` | правило про формат ответа ссылается на несуществующий документ | Комментарии: правда, польза, объем | partly-wrong |  |
| Открыто | НИЗКАЯ | `false-bbcode-pipeline-doc` | два комментария ссылаются на документ и разделы, которых нет | Комментарии: правда, польза, объем | severity-corrected |  |
| Открыто | НИЗКАЯ | `false-comment-vue-refs` | четыре комментария сверяют геометрию с файлом, которого нет | Комментарии: правда, польза, объем | severity-corrected |  |
| Открыто | НИЗКАЯ | `false-endorsement-composable` | бекенд-контракт сверяется с FE-композаблом, которого нет | Комментарии: правда, польза, объем | severity-corrected |  |
| Открыто | НИЗКАЯ | `false-profileendorsements-precedent` | ссылка на прецедент, которого нет | Комментарии: правда, польза, объем | partly-wrong |  |
| Открыто | НИЗКАЯ | `false-toolbar-exception-count` | обоснование "5 функций" называет тег, удаленный в том же файле | Комментарии: правда, польза, объем | severity-corrected |  |
| Открыто | НИЗКАЯ | `false-useunmounted` | тест объясняет свою настройку через несуществующий символ | Комментарии: правда, польза, объем | partly-wrong |  |
| Открыто | НИЗКАЯ | `filter-hints-drift` | подсказки фильтра перечисляют не те варианты, что в списке | Копирайт и язык интерфейса | severity-corrected |  |
| Открыто | НИЗКАЯ | `filter-props-declared-twice` | 7 интерфейсов пропсов дублируют defineProps, два уже разошлись | Гигиена кода и техдолг | severity-corrected |  |
| Открыто | НИЗКАЯ | `fixture-seed-bare-any-guards` | восемь сидеров фикстуры из тринадцати проверяют голым Any() | Стратегия и качество тестов | severity-corrected |  |
| Открыто | НИЗКАЯ | `four-caching-mechanisms` | четыре независимых механизма кэширования и два вида guard | Архитектура фронтенда (FSD) | partly-wrong |  |
| Открыто | НИЗКАЯ | `frontend-page-pairs` | шесть пар почти одинаковых файлов, 971 строка на одной стороне | Гигиена кода и техдолг | partly-wrong |  |
| Открыто | НИЗКАЯ | `game-role-derived-four-ways` | роль в игре выводится четырьмя способами | Предметная модель и продуктовая логика | partly-wrong |  |
| Открыто | НИЗКАЯ | `gamepanel-in-entry-chunk` | самая крупная условная панель сайдбара статична, вопреки правилу тремя строками ниже | Архитектура фронтенда (FSD) | partly-wrong |  |
| Открыто | НИЗКАЯ | `global-components-ungated` | девять компонентов зарегистрированы глобально, и ничто не проверяет, что шаблоны их находят | Архитектура фронтенда (FSD) | severity-corrected |  |
| Открыто | НИЗКАЯ | `image-size-defaults-declared-three-times-none-authoritative` | 600×400 в C#, 600×400 в TS, 100%×500px в CSS | Гигиена кода и техдолг | confirmed |  |
| Открыто | НИЗКАЯ | `include-ignored-by-projection` | два `Include`, которые EF выбрасывает | Реляционное хранилище и EF Core | confirmed |  |
| Открыто | НИЗКАЯ | `is-own-game-defined-by-exclusion` | предикат «своя игра» задан отрицанием | Предметная модель и продуктовая логика | severity-corrected |  |
| Открыто | НИЗКАЯ | `langversion-preview-defeats-a-pinned-setting` | два проекта переопределяют намеренно закрепленную версию языка | Архитектура бэкенда и границы модулей | severity-corrected |  |
| Открыто | НИЗКАЯ | `last-comment-denormalization` | денормализованный указатель, у которого единственный читатель это он сам | Реляционное хранилище и EF Core | partly-wrong |  |
| Открыто | НИЗКАЯ | `leaderboard-top-n-is-a-literal-eight-times` | `.Take(10)` без имени | Гигиена кода и техдолг | confirmed |  |
| Открыто | НИЗКАЯ | `likes-index-order-justification-overstated` | порядок колонок выбран верно, обоснование сильнее правды | Реляционное хранилище и EF Core | confirmed |  |
| Открыто | НИЗКАЯ | `lint-message-placeholder` | в каждом сообщении о нарушении границ пропущен объект импорта | Архитектура фронтенда (FSD) | confirmed |  |
| Открыто | НИЗКАЯ | `loading-copy-two-ellipses` | «Загрузка...» и «Загрузка…» вперемешку | Пользовательские сценарии | confirmed |  |
| Открыто | НИЗКАЯ | `login-reveals-account-state-without-password` | вход отвечает о существовании аккаунта до проверки пароля | Безопасность | partly-wrong |  |
| Открыто | НИЗКАЯ | `mail-subject-conventions` | пять форматов тем писем и одна эмодзи | Копирайт и язык интерфейса | severity-corrected |  |
| Открыто | НИЗКАЯ | `map-error-code-diverged` | две копии одного маппинга статуса в одном слайсе, уже разные | Архитектура фронтенда (FSD) | partly-wrong |  |
| Открыто | НИЗКАЯ | `mapping-config-duplication` | 58 файлов делают одну и ту же проверку на подвыборках | Стратегия и качество тестов | partly-wrong |  |
| Открыто | НИЗКАЯ | `mentor-counts-as-own-space` | код и SSOT расходятся в семантике бана | Безопасность | severity-corrected |  |
| Открыто | НИЗКАЯ | `message-layouts-declared-three-times` | комментарий обещает расширяемость, которой нет | Гигиена кода и техдолг | confirmed |  |
| Открыто | НИЗКАЯ | `metric-unit-in-name` | имя гистограммы длительности противоречит соглашению, на которое ссылается ее собственный комментарий | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | partly-wrong |  |
| Открыто | НИЗКАЯ | `middot-and-guillemets` | один "·" в живом UI и три пары елочек в конвенциях | Копирайт и язык интерфейса | confirmed |  |
| Открыто | НИЗКАЯ | `misplaced-jsdoc-formatthreshold` | док функции стоит над другой функцией | Комментарии: правда, польза, объем | partly-wrong |  |
| Открыто | НИЗКАЯ | `mixed-language-user-strings` | четыре строки на смеси языков или не на том языке | Копирайт и язык интерфейса | не проверялась |  |
| Открыто | НИЗКАЯ | `modblock-invariant-has-26-call-sites-and-no-test` | правило снятия привилегированной разметки держится на памяти | Гигиена кода и техдолг | partly-wrong |  |
| Открыто | НИЗКАЯ | `module-composition-is-a-hand-kept-list` | состав доменных модулей перечислен вручную в трех хостах, и пропуск ничего не ломает при сборке | Архитектура бэкенда и границы модулей | partly-wrong |  |
| Открыто | НИЗКАЯ | `mongo-collections-without-retention` | у двух коллекций нет ни удаления, ни TTL | Полиглотное хранение (PG / Mongo / MinIO) | severity-corrected |  |
| Открыто | НИЗКАЯ | `mongo-model-carries-relational-annotation` | реляционные атрибуты и имена полей хранилища в документной модели | Полиглотное хранение (PG / Mongo / MinIO) | confirmed |  |
| Открыто | НИЗКАЯ | `monitoring-doc-numbers-wrong` | четыре порога, число правил, gRPC-хост и все примеры PromQL не соответствуют коду | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | confirmed |  |
| Открыто | НИЗКАЯ | `muted-class-double-meaning` | один класс означает две разные вещи | Качество фронтенда: дизайн-система, a11y, производительность | partly-wrong |  |
| Открыто | НИЗКАЯ | `naming-drift` | одно понятие под двумя именами в шести местах | Гигиена кода и техдолг | partly-wrong |  |
| Открыто | НИЗКАЯ | `narrative-comments` | жанр "история правки" вместо описания текущего кода | Комментарии: правда, польза, объем | partly-wrong |  |
| Открыто | НИЗКАЯ | `nine-hand-rolled-timer-loops` | девять почти одинаковых циклов таймера с расходящейся структурой | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | partly-wrong |  |
| Открыто | НИЗКАЯ | `no-serilog-flush` | последние строки лога теряются на каждом останове, а у миграционного контейнера теряется весь прогон | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | partly-wrong |  |
| Открыто | НИЗКАЯ | `notification-labels-duplicated` | 48 подписей в двух файлах и одно плечо расхождения | Гигиена кода и техдолг | severity-corrected |  |
| Открыто | НИЗКАЯ | `notification-rules-live-in-the-worker` | 41 генератор уведомлений, 39 держат DmDbContext, ноль вызывают доменный сервис | Архитектура бэкенда и границы модулей | partly-wrong |  |
| Открыто | НИЗКАЯ | `notifications-grow-forever` | у потока уведомлений нет ни срока хранения, ни ограничения | Асинхронный обмен сообщениями и realtime | severity-corrected |  |
| Открыто | НИЗКАЯ | `object-key-not-a-hash` | "hash-based, immutable" на самом деле случайные 8 hex | Полиглотное хранение (PG / Mongo / MinIO) | confirmed |  |
| Открыто | НИЗКАЯ | `orthography-and-quoted-labels` | дефисы и регистр в цитируемых названиях | Копирайт и язык интерфейса | confirmed |  |
| Открыто | НИЗКАЯ | `otmena-vs-otmenit` | в диалогах бана и предупреждения кнопка закрытия названа "Отменить" | Копирайт и язык интерфейса | severity-corrected |  |
| Открыто | НИЗКАЯ | `own-config-breaks-the-projects-text-rules` | 14 "е с точками" в файлах .claude | Процесс разработки: человек и Claude | partly-wrong |  |
| Открыто | НИЗКАЯ | `own-space-predicate-triplicated` | «своя область» определена трижды | Предметная модель и продуктовая логика | severity-corrected |  |
| Открыто | НИЗКАЯ | `owner-rules-outside-repo` | часть жестких правил владельца не существует ни в одном файле репозитория | Документация | partly-wrong |  |
| Открыто | НИЗКАЯ | `pagination-label-and-politeness` | два слова для одного действия и одно "пожалуйста" | Копирайт и язык интерфейса | partly-wrong |  |
| Открыто | НИЗКАЯ | `patterns-contradicts-itself-on-cross-module-calls` | чертеж утверждает две противоположные вещи о вызовах между модулями | Архитектура бэкенда и границы модулей | partly-wrong |  |
| Открыто | НИЗКАЯ | `performance-unmarked-aspirations` | в одном списке лежат действующие правила и никогда не принятые практики | Документация | confirmed |  |
| Открыто | НИЗКАЯ | `persistence-tier-is-mostly-mapping-config` | 71 репозиторий, 5 с прямым тестом | Стратегия и качество тестов | partly-wrong |  |
| Открыто | НИЗКАЯ | `poll-entity-in-the-wrong-module-folder` | размещение сущности не по собственному правилу документа | Полиглотное хранение (PG / Mongo / MinIO) | confirmed |  |
| Открыто | НИЗКАЯ | `popularity-rule-duplicated-and-diverging` | правило популярности написано дважды, константа разошлась, а лог утверждает неправду про первый расчет | Архитектура бэкенда и границы модулей | partly-wrong |  |
| Открыто | НИЗКАЯ | `post-attachments-public-bucket` | вложения закрытых комнат отдаются анонимно | Безопасность | partly-wrong |  |
| Открыто | НИЗКАЯ | `publication-draft-not-cleared` | после публикации черновик остается и предлагается снова | Пользовательские сценарии | severity-corrected |  |
| Открыто | НИЗКАЯ | `redundant-nullable-directives-and-cs1591-split` | двенадцать no-op директив и две политики документирования в одной папке | Гигиена кода и техдолг | confirmed |  |
| Открыто | НИЗКАЯ | `redundant-work-in-hot-paths` | глубокий watch на списке сообщений и неpassive-слушатель прокрутки | Качество фронтенда: дизайн-система, a11y, производительность | partly-wrong |  |
| Открыто | НИЗКАЯ | `reloadonchange-without-monitor` | перезагрузка секретов включена, но читать обновление нечем | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | confirmed |  |
| Открыто | НИЗКАЯ | `reminder-published-before-save` | напоминания о постах публикуются до того, как отметка о них станет durable | Асинхронный обмен сообщениями и realtime | severity-corrected |  |
| Открыто | НИЗКАЯ | `replacement-chars-in-startup` | в комментариях композиционного корня закоммичен U+FFFD | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | confirmed |  |
| Открыто | НИЗКАЯ | `retirement-flags-unconstrained` | три булевых флага уточняют один статус, и ничто их не разводит | Предметная модель и продуктовая логика | severity-corrected |  |
| Открыто | НИЗКАЯ | `roadmap-vs-mirroring-session-transfer` | два документа противоречат друг другу о переносе сессий | Документация | partly-wrong |  |
| Открыто | НИЗКАЯ | `room-access-user-deref-asymmetric` | одна и та же проверка написана двумя способами | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | НИЗКАЯ | `russian-comments-in-code` | 37 строк в 17 файлах против правила проекта | Гигиена кода и техдолг | partly-wrong |  |
| Открыто | НИЗКАЯ | `signout-leaves-protected-page` | после выхода со страницы настроек остается пустой экран с заголовком | Пользовательские сценарии | severity-corrected |  |
| Открыто | НИЗКАЯ | `soft-delete-audit-columns-unwritten` | `ISoftDeletable` обещает автора удаления, большинство путей его не пишет | Реляционное хранилище и EF Core | severity-corrected |  |
| Открыто | НИЗКАЯ | `split-clock-in-inactivity` | правило неактивности берет время из двух источников | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | НИЗКАЯ | `split-query-offset-comment-overstates` | заявлено, что лечится только keyset, хотя лекарство рядом | Реляционное хранилище и EF Core | confirmed |  |
| Открыто | НИЗКАЯ | `ssrf-blocklist-prefix-based` | блок-лист хостов обходится и защищает не то, что заявлено | Безопасность | confirmed |  |
| Открыто | НИЗКАЯ | `stale-cross-references` | три комментария ссылаются на то, чего нет | Полиглотное хранение (PG / Mongo / MinIO) | confirmed |  |
| Открыто | НИЗКАЯ | `stale-dto-see-paths` | десять `@see` в рукописных FE-зеркалах C#-DTO ведут в несуществующий каталог | Комментарии: правда, польза, объем | severity-corrected |  |
| Открыто | НИЗКАЯ | `stale-thumbnail-comments` | пять мест обещают превью `_m`/`_s`, которых нет | Полиглотное хранение (PG / Mongo / MinIO) | confirmed |  |
| Открыто | НИЗКАЯ | `storage-lifecycle-bypasses-the-clock-abstraction` | сроки хранения считаются от системных часов | Полиглотное хранение (PG / Mongo / MinIO) | confirmed |  |
| Открыто | НИЗКАЯ | `subscription-settings-comment-wrong` | док утверждает совпадение битов, которого нет | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | НИЗКАЯ | `subscriptions-page-guids-and-broken-links` | страница подписок показывает GUID и три ссылки из четырех ведут в никуда | Пользовательские сценарии | severity-corrected |  |
| Открыто | НИЗКАЯ | `system-role-outranks-admin` | робот старше администратора во всех сравнениях | Предметная модель и продуктовая логика | confirmed |  |
| Открыто | НИЗКАЯ | `tap-targets-under-24` | иконочные цели меньше минимума WCAG 2.5.8 | Качество фронтенда: дизайн-система, a11y, производительность | partly-wrong |  |
| Открыто | НИЗКАЯ | `three-precommit-checklists` | "перед коммитом" существует в трех несовпадающих версиях | Документация | severity-corrected |  |
| Открыто | НИЗКАЯ | `token-include-loads-full-user` | в память тянутся хеши паролей, а картинка приглашенного теряется | Реляционное хранилище и EF Core | confirmed |  |
| Открыто | НИЗКАЯ | `token-pages-blank-while-loading` | две страницы по ссылке из письма не показывают ничего, пока проверяют токен | Пользовательские сценарии | severity-corrected |  |
| Открыто | НИЗКАЯ | `tooltipcontent-dom-surgery` | подмена узла через `outerHTML` за спиной Vue, плюс отладочный текст в проде | Качество фронтенда: дизайн-система, a11y, производительность | partly-wrong |  |
| Открыто | НИЗКАЯ | `two-ellipsis-forms` | многоточие набирается двумя разными символами | Копирайт и язык интерфейса | severity-corrected |  |
| Открыто | НИЗКАЯ | `two-sources-of-truth-for-environment` | имя окружения читается из переменной среды напрямую, минуя IHostEnvironment | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | severity-corrected |  |
| Открыто | НИЗКАЯ | `ui-standards-centralization-overstated` | шкалы объявлены централизованными, код утверждает обратное | Документация | partly-wrong |  |
| Открыто | НИЗКАЯ | `unbound-options-in-secondary-hosts` | два хоста регистрируют доменные сборки, не привязав их опции, вопреки заявленному инварианту | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | severity-corrected |  |
| Открыто | НИЗКАЯ | `uneven-guard-annotation` | из четырех одинаковых проверок объяснены две | Комментарии: правда, польза, объем | confirmed |  |
| Открыто | НИЗКАЯ | `unmarked-deferral` | отложенная работа записана прозой, поэтому не находится grep-ом | Комментарии: правда, польза, объем | confirmed |  |
| Открыто | НИЗКАЯ | `unvalidated-config-sections` | половина разделов привязана без проверки, и неверное значение всплывает на первом запросе | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | partly-wrong |  |
| Открыто | НИЗКАЯ | `unwrap-inlined` | хелпер распаковки конверта продублирован в файлах, которые его же импортируют | Архитектура фронтенда (FSD) | partly-wrong |  |
| Открыто | НИЗКАЯ | `user-settings-two-owners` | один документ, четыре обращения, две несовместимые стратегии записи | Полиглотное хранение (PG / Mongo / MinIO) | partly-wrong |  |
| Открыто | НИЗКАЯ | `validateonstart-defeated-by-checked-in-defaults` | заявленное "отсутствие секрета валит старт" не работает ни для базы, ни для MinIO | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | partly-wrong |  |
| Открыто | НИЗКАЯ | `vite-config-comments-lie` | два комментария в конфиге сборки не соответствуют коду | Архитектура фронтенда (FSD) | confirmed |  |
| Открыто | НИЗКАЯ | `webhook-error-shape` | ответы вебхука используют форму ошибки, от которой проект отказался | Асинхронный обмен сообщениями и realtime | confirmed |  |
| Открыто | НИЗКАЯ | `yo-and-quotes-in-agent-docs` | файлы, задающие текстовые правила, их же нарушают | Документация | partly-wrong |  |
| Открыто | НИЗКАЯ | `zindex-off-scale` | три места вне шкалы, одно инвертирует ее порядок | Качество фронтенда: дизайн-система, a11y, производительность | partly-wrong |  |
...
| Опровергнуто | СРЕДНЯЯ | `xmldoc-ceremony` | гейт требует наличия дока, поэтому больше половины XML-дока это синтаксис | Комментарии: правда, польза, объем | matter-of-taste | Находки не существует |
| Опровергнуто | НИЗКАЯ | `character-attributes-n-plus-1` | по запросу на каждый атрибут при сохранении персонажа | Реляционное хранилище и EF Core | refuted | Находки не существует |
| Опровергнуто | НИЗКАЯ | `greeting-register` | приветствие в трех регистрах | Копирайт и язык интерфейса | matter-of-taste | Находки не существует |
| Опровергнуто | НИЗКАЯ | `password-reset-success-no-login-link` | после смены пароля нет кнопки войти | Пользовательские сценарии | matter-of-taste | Находки не существует |
| Опровергнуто | НИЗКАЯ | `patterns-names-concrete-types` | SSOT по структуре ссылается на конкретные имена и пути | Архитектура бэкенда и границы модулей | matter-of-taste | Находки не существует |
| Опровергнуто | НИЗКАЯ | `popularity-log-lies` | строка лога сообщает о начальном расчете, которого никто не делает | Композиционный корень, конфигурация, hosted-сервисы, наблюдаемость | refuted | Находки не существует |
| Опровергнуто | НИЗКАЯ | `required-validator-no-message` | обязательность поля не объясняется словами | Копирайт и язык интерфейса | matter-of-taste | Находки не существует |
| Опровергнуто | НИЗКАЯ | `yo-in-claude-docs` | 15 вхождений буквы "е с двумя точками" в трех файлах .claude | Копирайт и язык интерфейса | refuted | Находки не существует |
| Опровергнуто | ОПРОВЕРГНУТО | `notification-metadata-anonymous-type` | `Notification.Metadata` хранит анонимный тип, который драйвер отказывается сериализовать | Полиглотное хранение (PG / Mongo / MinIO) | опровергнуто аудитом | Находки не существует |
| Опровергнуто | ОПРОВЕРГНУТО | `private-tag-unquoted-not-parsed` | приватный текст в формате редактора виден всем | Безопасность | опровергнуто аудитом | Находки не существует |
...
Находки, которых аудит не нашел. Каждая появилась, когда гейт для другой находки первым
исполнил соответствующий код, а не при чтении.
