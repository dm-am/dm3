...
искать), как `test` сочетается с гейтами pre-push, и как обновлять скиллы из
апстрима. Агент вызывает скиллы сам, когда задача совпадает с описанием;
ручной вызов `/имя` — только чтобы заставить.
