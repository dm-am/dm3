...
**Сессия между зеркалами не переносится.** Кука привязана к домену, поэтому браузер не отдает ее другому зеркалу, и вход выполняется на каждом отдельно. Переключатель региона переводит на тот же путь другого зеркала, не более. Это закрытое решение, а не отложенная задача: перенос потребовал бы выдать токен сессии скрипту по неаутентифицированному вызову и пронести токен переноса в URL.
