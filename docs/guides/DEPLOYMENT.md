...
Периодические задания не единственная причина. Realtime-пуш адресует соединения
по карте, которая живет в памяти процесса, и читает свою очередь эксклюзивно:
второй экземпляр не увидел бы соединений первого, а очередь ему объявить не
дадут. Часть читателей осталась бы без пушей, и оба экземпляра считали бы, что
все в порядке.

Снимается это по частям, и каждая часть — отдельная работа: задания вынести
в собственный процесс (как уже сделано с почтой и уведомлениями) либо закрыть
распределенной блокировкой, а пушам нужны общая для экземпляров карта соединений
и своя очередь на каждый экземпляр. Пока ничего из этого нет, `deploy.replicas`
для API — заявка, которую нечем обеспечить.
