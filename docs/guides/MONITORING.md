...
flowchart LR
    ...
    jaeger --> grafana
    prom --> grafana
    prom -->|"сработавшее правило"| alerts["Alertmanager :9093"]
    alerts -->|"письмо получателю"| smtp["SMTP relay<br/>тот же, что у писем сайта"]
```
...
настройки. На стенде relay это Mailpit, и письмо видно на http://localhost:8025.
...
без него останавливается. Дефолт compose это Mailpit, слушающий петлю стенда, и
...
| Mailpit (стенд) | http://localhost:8025 |
...
1. Проверить, что Jaeger запущен и его проба зеленая:
   `docker inspect --format '{{.State.Health.Status}}' dm-jaeger`
2. Проверить, что трассы вообще доходят:
   `curl http://localhost:16686/api/services` — в списке должен быть `DM.API`.
   Пустой список при живом приложении означает, что до коллектора не доехало
   ничего, а не что искомая трасса потерялась
...
1. **Опрашивать `https://<адрес>/_ready`**, а не `/_health`. Живость отвечает
   ...
   спрашивает Postgres. Порог — две-три неудачные пробы подряд, иначе
