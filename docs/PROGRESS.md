...
flowchart LR
    ...
    G -.->|"живой проход"| G2["Правки по фидбеку яруса 1"]
    U -.->|"живой проход"| U2["Правки по фидбеку яруса 2"]
    M -.->|"живой проход"| M2["Правки по фидбеку яруса 3"]
```
...
| Зона | Статус | Заметки |
|------|--------|---------|
| Аутентификация | ✅ / 🔎 | вход, регистрация, восстановление, сброс, подтверждение; honeypot + тайминг антибот; вход в два шага, если включен второй фактор |
| Настройки аккаунта | ✅ / 🔎 | смена имени через модерацию; второй фактор: включение по QR или строке секрета, резервные коды, отключение и перевыпуск |
| Личные сообщения | ✅ / 🔎 | |
...
| Цитирование | ✅ / 🔎 | кнопка у поста, комментария, топика и реплик обоих чатов; разметку цитаты собирает сервер, уже отфильтрованную для читателя |
...
| Снятие второго фактора у коллеги | ✅ / 🔎 | вторым администратором с профиля; единственный путь для рангов, которым фактор обязателен |
...
прохода тут не хранится: он устаревает к следующим правкам, а найденное живет в
...
профиль, вкладка теряется), `/activate`, `/confirm-email`, `/reset-password`,
`/change-username`, `/two-factor/removal`, `/two-factor/removal/cancel`,
`/auth/callback`, `/error/:code?`, переходы к непрочитанному
(`/forum/:alias/:num/unread`, `/game/:id/posts/unread`,
`/game/:id/comments/unread`) и редиректы `/forum-topic/:topicId` и
`/publication/:pubId`.

Шесть из них ведут по ссылке из письма и открыты гостю намеренно: активация,
подтверждение почты, сброс пароля, выбор нового имени, снятие второго фактора и
отказ от снятия. Письмо часто открывают там, где сессии нет, а тому, кто снимает
фактор, войти уже нечем.
...
| Редакторы тех же мест | кнопка `[mod]`, разметка публична на чтение |
...
Числа тестов и объемы сида здесь не хранятся: они меняются с каждой правкой и
