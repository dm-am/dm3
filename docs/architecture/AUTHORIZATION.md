...
`[private]` и `[mod]` — privacy-sensitive BBCode-теги, видимость которых определяется правами зрителя и контекстом контента. Фильтрация происходит на сервере **до сериализации JSON**: отфильтрованный контент никогда не попадает на клиент к неавторизованному зрителю.

**`[mod]`** — виден только `UserRole >= Moderator`, в surface'ах ForumTopic / Comment / GlobalChatMessage. Вне этих surface'ов безусловно вырезается.
