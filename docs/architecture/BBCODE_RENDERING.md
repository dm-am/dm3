...
Каждая строка BBCode принадлежит одной семантической поверхности, определяющей допустимый набор тегов. Surface задаётся на уровне маппера при сериализации и проверяется на write-time (невалидный тег для surface → `BadRequestError`).
...
Попытка использовать тег вне его surface — ошибка валидации при сохранении. Defense-in-depth: renderer никогда не эмитит privacy-sensitive тег из surface, где он не разрешён, даже если каким-то образом такой тег оказался в тексте.

## Audience — семантическое намерение рендера

Audience — это **намерение**, а не формат. Клиент передаёт audience в HTTP-заголовке при запросе, сервер рендерит под выбранное намерение.
...
`[private]` валиден только в GamePost. Правило видимости — **OR-объединение** следующих условий (любое из них даёт доступ):
...
| `author_edit` | автор (подтверждён endpoint-уровнем) | ✅ виден с `data-bb-tag="private" data-bb-addressees="..."` |
| `plain_text` / `embed_safe` | любое | ❌ вырезан |

## Правило ведущих игры

**Ведущие игры = `GameRole.Master` + `GameRole.Assistant`.** Ментор (`GameRole.Mentor`) **не** является ведущим — он просто помогает участникам игры, не ведёт её. Это правило фиксировано на уровне render-контракта: коллекция `GameLeadUserIds` в контексте рендера включает master + assistants и **не** включает менторов.
...
Вне зависимости от активного режима, на сохранение идёт BBCode-представление.
...
Viewer в envelope **не кладётся** — он резолвится per-request из authorization context. Envelope безопасно делить между пользователями: он не содержит per-user данных.
...
Email и прочие небезопасные каналы (нотификации, поисковый индекс) рендерят только `plain_text` audience: privacy-теги вырезаются безусловно, без учёта получателя. Причина: email'ы пересылаются, кэшируются, архивируются — любая попытка per-recipient редакции не выдерживает критики. Снимок на момент отправки считается окончательным.
