...
Семантический токен тоста объявляется парой: значение для светлой темы и значение
для темной. Пара заводится сразу целиком, значения берутся из палитры темы, а не
пишутся в компоненте. Контраст текста к фону проверяется в каждой теме отдельно:
пара, прошедшая проверку в светлой, в темной ее не наследует, и как раз на желтом
фоне предупреждения текст меняет светлоту.
