...
| Контейнер, отступ которому задают дети | `+panel()` — то же без отступа |
...
Текст интерфейса выделяется и копируется целиком. `user-select: none` разрешен ровно в одном месте — у горизонтального `DashSeparator`, где "- - - -" это линия, а не слова. Декоративный префикс "- " в строках сайдбара несет `aria-hidden`, но выделяется и копируется вместе со строкой: правильная вставка "- Игры", а не "Игры". Однострочная композиция обязана копироваться одной строкой, без склеек и лишних пробелов: подпись ссылки пишется вплотную к тегу, иначе компилятор шаблона добавляет к ней ведущий пробел.
