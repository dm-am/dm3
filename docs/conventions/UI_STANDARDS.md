...
| `$bp-tablet` | 768px | Реflow контентных сеток (сброс колонки) |
| `$bp-mobile` | 600px | Однoколоночная верстка, компактные контролы |
...
На данный момент по токенам мигрирован только шелл (`App.vue`, `Header.vue`) и мобильный drawer — остальные компоненты с захардкоженными 768/640/600 и т.п. не трогались (будущая консолидация).
...
Ссылки-названия игр и блогов раскрашиваются по статусу единообразно везде (сайдбары, таблицы /games и /blogs, профильные таблицы, хлебные крошки постов): **зелёный** ($accent-green) — «новый» модуль (активирован менее 7 дней назад и не закрыт); **приглушённый серый** ($text-muted, до hover) — Closed; остальное (Draft, активные старше 7 дней) — обычный $link. Серый приоритетнее зелёного. Архивные комнаты трактуются как закрытые — тем же приглушённым серым; архивность передаётся цветом (и, где уместно, тегом «архив»), НИКОГДА суффиксом в самом названии. Текстовые бейджи статуса (GameStatusBadge/BlogStatusBadge) цвета не несут.
