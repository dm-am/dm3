...
| **Транзакции** | Несколько операций должны выполниться "всё или ничего" (ACID) |
...
Entities группируются **по модулям** (папкам), повторяя структуру Domain-слоя. Если entity используется несколькими модулями — она идёт в **Shared Kernel**.
...
// EF Core автоматически фильтрует "удалённые" записи
...
Надёжная отправка событий: сначала событие сохраняется в таблицу `OutboxEvents`, затем отдельный процесс отправляет его в очередь сообщений. Гарантирует, что событие не потеряется даже при сбое.

---

## Миграции

**Правило:** существует только одна миграция `InitialCreate`. Все изменения схемы вносятся в неё, новые миграции не создаются.
