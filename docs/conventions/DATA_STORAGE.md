...
Надежная отправка событий: сначала событие сохраняется в таблицу `OutboxEvents`, затем отдельный процесс отправляет его в очередь сообщений. Гарантирует, что событие не потеряется даже при сбое.
