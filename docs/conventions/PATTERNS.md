...
- Каждый слайс `entities/{x}`, `features/{x}`, `widgets/{x}` экспортирует наружу **только через свой barrel** `index.ts`. Deep-import мимо barrel во внутренние файлы слайса запрещен — потребитель не знает внутреннюю раскладку.
- У `pages/{x}` barrel не заводится: у страницы один потребитель, роутер, и он обязан адресовать файл страницы напрямую, иначе разбиение на чанки исчезает.
...
Глобальная регистрация компонентов (`app/providers/components.ts`) — единственная
лазейка мимо этого гейта: у глобального компонента нет импорта, и связь через
него линтер не видит. Поэтому регистрировать глобально можно только примитивы
`shared`; доменный компонент в этом списке был бы same-layer зависимостью мимо
`@x` при зеленых гейтах. Правило закреплено тестом `fsdBoundaries.spec.ts`.
...
Одна папка — одна область контракта. Здесь допустимы чистые политики и хелперы без
состояния и без обращений наружу: санитайзер разметки, кодирование публичного
идентификатора, сравнение ролей. Все, чему нужны хранилище, сеть, часы или собственное
состояние, живет в модуле.
...
### ❌ "Модуль вызывает сервис соседнего модуля напрямую"

Нет, и правило одностороннее. Синхронное чтение чужой области разрешено, но только через
контракт в Domain.Core: потребность объявляет ядро, реализует модуль-владелец, ссылки
между `Domain.*` не появляется ни в одну сторону. Запись и побочные эффекты синхронно не
вызываются: уведомление, начисление, рассылка уходят Domain Event, и владелец решает, что
с ним делать. Обе половины проверяются архитектурным тестом.
...
### ❌ "Domain.Core может содержать сервис предметной области"

Нет. Domain.Core — это **Shared Kernel**: интерфейсы, DTO, enums, exceptions и чистые
политики, нужные больше чем одному модулю. Граница проходит по зависимостям, а не по слову
"логика": у ядра нет ни одной ссылки на проект или пакет, и это проверяется архитектурным
тестом. Сервис с состоянием или с обращением наружу принадлежит модулю.
