...
- Общие DTO и exceptions, enums домена целиком (в том числе те, что читает один модуль)
...
**Граница между двумя слоями сервисов.** Доменный сервис владеет правилами: авторизация, инварианты, работа с репозиториями, публикация событий. API-сервис владеет формой HTTP-ответа: маппинг в DTO, конверт, сборка ответа из нескольких доменных вызовов, разрешение публичного идентификатора. Правило одностороннее: правило предметной области в API-сервисе — дефект, а знание о конверте и DTO в доменном сервисе — тоже дефект. Если API-сервису нечего делать кроме прямого проброса, он все равно нужен: он держит форму ответа стабильной, когда доменная сигнатура меняется. Граница проверяется архитектурным тестом, а не на ревью: контроллер не вправе зависеть ни от одной абстракции `DM.Domain.*`, а API-сервис — от `DmDbContext` и от любого контракта, который реализует слой хранения.
...
в `eslint.config.js` — исполняемая копия этого раздела: направление импортов, запрет
...
```
DM.Domain.Core/
├── Enums/                # enums домена: виды, статусы, роли, режимы, параметры запроса
└── {Concern}/            # интерфейсы, DTO и enums своего контракта
```
...
**Enums домена объявляет ядро, и лежат они в `DM.Domain.Core/Enums`** — словарь, которым
домен называет сущности, их виды, статусы, роли, режимы доступа и настройки, а также
значения, которыми запрос описывает отбор, сортировку и обход. Причина в графе
зависимостей: словарь читает больше одного модуля, модули друг на друга не ссылаются, и
ядро остается единственным местом, которое видно всем сразу. Ядро для этого и годится: у
него нет ни одной ссылки на проект или пакет, поэтому объявление в нем ничего за собой не
тянет.

Правило шире сегодняшнего состава читателей, и это выбор, а не недосмотр: часть того, что
лежит в `Enums/`, сегодня читает ровно один модуль, и обратно в модули оно не переносится.
Цена названа: теряется сигнал владения, а правка любого перечисления перекомпилирует все
модули. Взамен место объявления одно и проверяется взглядом на каталог, а перечисление не
переезжает через границу проекта в тот день, когда у него появляется второй читатель. Ни
число читателей, ни то, что перечисление обслуживает один запрос или один контракт, из
`Enums/` его не выселяют.

Вне `Enums/` ядро объявляет перечисление там, где папка держит его по своей конвенции:
намерение авторизации — в `Authorization/`, значение конфигурационной опции — в
`Configuration/` рядом с классом конфигурации, запрошенный переход состояния — в
`Statuses/`, исход операции — рядом с ее контрактом.

Модуль объявляет перечисление у себя, когда оно неотделимо от его собственного контракта
и ни один другой Domain-модуль его не читает: намерение `{Entity}Intention`, переход
`{Entity}StatusTransition`, исход операции, параметр собственного запроса. Правило работает
в одну сторону: оно разрешает модулю держать свое, но ничего не выселяет из `Enums/`. Как
только читателей у перечисления становится двое, объявление переезжает в ядро.
...
├── Authorization/
├── Configuration/
├── {Module}RegistrationExtensions.cs       # AddDm{Module}: регистрации сверх умолчания скана
└── {Module}ConfigurationExtensions.cs      # привязка опций модуля
```

Два корневых файла заводятся по потребности. `{Module}RegistrationExtensions.cs`
появляется, когда типам модуля нужно время жизни, которого скан сборок по
умолчанию не дает, а `{Module}ConfigurationExtensions.cs` — когда привязку своих
опций модуль берет на себя, а не оставляет хосту.

Скан сборки (`AddDefaultTypes`) подхватывает каждый класс, все объявленные
конструкторы которого публичны. Тип выводится из скана объявлением непубличного
конструктора, и это единственный принятый способ opt-out: контейнер проверяет
весь граф при старте (ValidateOnBuild), поэтому регистрация класса, аргументы
которого контейнер не поставляет, роняет хост, а не лежит мертвым грузом.
Идиома применяется, когда экземпляры строит рука или фабрика, а не контейнер:
вспомогательный класс с данными в аргументах, типизированный HTTP-клиент,
которого конструирует AddHttpClient, форма ответа API. Причина каждого такого
конструктора лежит комментарием рядом с ним.

**Исключение: под-ресурс остается в папке родителя, когда вынос приходится
оплачивать.** Признак составной, и нужны оба условия сразу: путь чтения родителя
сам заполняет производные значения под-ресурса, и каждая операция под-ресурса
начинается с чтения родителя. У выноса тогда два исполнения, и оба дороже, чем
оставить как есть. Забрать заполнение производных значений с собой значит дать
родителю зависимость от нового сервиса, а новому сервису зависимость от
родителя. Такую пару контейнер не построит. Оставить заполнение приватным
хелпером родителя значит развести одно вычисление по двум местам, где оно
разойдется молча. Под-ресурс, который только читает родителя, а родитель о нем
ничего не спрашивает, выносится обычным порядком.

Исключение не подразумевается, а объявляется: там, где оно применено, причина
лежит комментарием рядом с кодом. Невысказанное исключение читается как
недоделанный вынос, и следующий проход по коду заводит папку заново.
...
└── {Name}RegistrationExtensions.cs  # AddDm{Name}: DI-регистрации проекта
```

### Завендоренный чужой код

Библиотека, которую держим у себя вместо пакета, лежит отдельным проектом в `src/`,
а не папкой внутри своего потребителя. Проект дает то, чего папка не дает: свою
сборку, поэтому скан по конвенции не находит в ней второй ответ на наш интерфейс, и
свою полосу настроек компилятора, которой можно разрешить то, чего не разрешено
нашему коду. Приводить чужой код к нашим правилам нельзя: это переписывание чужой
библиотеки под политику, на которую она не подписывалась, и каждая правка — шанс
изменить то, что она делает с уже сохраненным текстом.

- Само решение вендорить принимает владелец. Ссылка на репозиторий, названная
  агентом или планом, разрешением не является.
- Имя сборки не меняется: переход с пакета на проект обязан остаться единственным,
  что случилось у потребителей.
- Ссылку на проект держит только тот слой инфраструктуры, который его оборачивает.
  Остальное дерево видит обертку, а не библиотеку.
- Взятое состояние объявляется в самом проекте: откуда исходники, под какой
  лицензией, какой коммит и почему именно он. Образец — `src/BBCodeParser/BBCodeParser.csproj`.
...
потребителю, который работает без HTTP-запроса, статус в исключении не значит
ничего.

Плата принята сознательно: перевод статуса в тело ответа собран в одном месте
(`ErrorHandlingMiddleware`), а фоновые потребители домена решают судьбу отказа
по самому факту исключения, а не по коду. Заводить вторую таксономию и таблицу
соответствия окупается тогда, когда статус понадобится прочитать за пределами
HTTP-хоста: второму транспорту со своим отображением ошибок или воркеру,
который по коду выбирает между повтором и немедленным отказом. Это и есть
триггер пересмотра, и проверяется он грепом по чтениям `StatusCode`, а не
толкованием слова "транспорт": вне `ErrorHandlingMiddleware` статус читает
только домен и только там, где "не найдено" от чужого поиска и есть предметный
ответ — пустой список, ноль, отсутствие записи.
...
политики, нужные больше чем одному модулю. Enums домена лежат в ядре целиком, даже когда
читатель у перечисления сегодня один: правило и его цена — в блюпринте выше. Граница
проходит по зависимостям, а не по слову "логика": у ядра нет ни одной ссылки на проект или
пакет, и это проверяется архитектурным тестом. Сервис с состоянием или с обращением наружу
принадлежит модулю.
...
**Маппинги — только Riok.Mapperly.** AutoMapper удален из стека целиком:
рантайм-маппера в приложении нет. Пакет Mapperly подключается с
`PrivateAssets="all"`: это source-generator, в рантайм он не попадает.

Валидация конфигурации живет в компиляторе, а не в тесте: RMG012/RMG020/RMG068
подняты до error в `.editorconfig` — незамапленный член или неинлайнящаяся
EF-проекция ломают сборку.

Идиомы:

- **Instance-мапер с ctor-инъекцией** — `[Mapper]` partial-класс рядом с фичей.
  Все, что AutoMapper решал резолверами (`IIdentityProvider`,
  `IDateTimeProvider`), становится зависимостями конструктора и обычными
  методами мапера. Контейнер подхватывает класс блочным сканом — отдельной
  регистрации нет.
- **AfterMap — явный вызов.** Дозаполнение результата (например,
  `RenderContextEnvelope`) — именованный метод, вызываемый после мапа в самом
  мапере или сервисе. Никаких неявных хуков.
- **Общие конверсии — статические хелперы** через
  `[UseStaticMapper(typeof(...))]`; для EF-проекций — статические
  expression-хелперы (образцы — `AvatarProjections`,
  `GeneralUserProjections`), не копипаста формул.
- **EF-проекции** — `static partial IQueryable<TDto> ProjectTo...(this
  IQueryable<TEntity>)` в статическом `[Mapper]`-классе; `TagWith` остается на
  вызывающей стороне. Проекция с вложенными пользователями или запросной
  логикой (OrderBy, флаттенинг, коррелированные подзапросы) пишется явным
  `Select`-выражением в статическом `...Mappers`-классе: Mapperly не инлайнит
  пользовательские методы в queryable-проекции (RMG068 поднят до error и
  ловит это на сборке). Общее правило, встраиваемое в несколько проекций,
  живет одним `Expression`-полем и вклеивается `ExpressionSplicer.Splice`
  (образцы — формула пользователя `GeneralUserProjections.Projection` и
  правило пендинга `PostPendencyFilters.OfRoom` в `GameMappers`); опаковый
  метод в EF-проекции запрещен — он заставляет провайдера материализовать
  строку целиком.
- **PATCH-семантика null:** null-коллекция или null-поле из запроса означает
  "не прислали", а не "очистить" — write-маппинг обязан сохранять null, и на
  каждую такую пару есть тест до миграции.
...
Разбор пользовательской разметки, границы между грамматикой, политикой и выводом и правило безопасного отказа — в [BBCODE_RENDERING.md](../architecture/BBCODE_RENDERING.md). Там же полный контракт: правила surface, audience-модель, матрица видимости privacy-тегов, permission-бакеты кэша, асимметричный контракт редактора.
