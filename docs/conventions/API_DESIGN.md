...
Прод-адрес API из таблицы выше — отдельный origin, поэтому браузер спрашивает
разрешение до самого запроса. Разрешение состоит из двух перечней, и оба ведутся
руками.
