...
public class GameReadingServiceShould : UnitTestBase
{
    ...
    public GameReadingServiceShould()
    {
        ...
        intentionManager
            .Setup(m => m.IsAllowed(It.IsAny<GameIntention>(), It.IsAny<Game>()))
            .Returns(true);

        var schemaService = Mock<ISchemaReadingService>();
        ...
        repository
            .Setup(r => r.GetPendingPosts(It.IsAny<IEnumerable<Guid>>(), It.IsAny<Guid>()))
            .ReturnsAsync(Array.Empty<PendingPost>());
        repository
            .Setup(r => r.GetRoomsAndPendingPosts(It.IsAny<IEnumerable<Guid>>(), It.IsAny<Guid>()))
            .ReturnsAsync((new Dictionary<Guid, IEnumerable<Guid>>(), Array.Empty<PendingPost>()));
        repository
    ...
    {
        // Arrange
        repository.Setup(r => r.GetOwn(testUserId)).ReturnsAsync(Array.Empty<Game>());
    ...
    public async Task ReturnGamesForUser()
    {
        ...
        var games = new[]
        {
            new Game { Id = game1Id, Title = "Test Game 1" },
            new Game { Id = game2Id, Title = "Test Game 2" }
        };
    ...
    public async Task CallBothUnreadCountersMethods()
    {
        ...
        var games = new[] { new Game { Id = gameId, Title = "Test Game" } };
        repository.Setup(r => r.GetOwn(testUserId)).ReturnsAsync(games);
    ...
    public async Task GetGames_CallsBothCountersInParallel_ForAuthenticatedUser()
    {
        ...
        var gameId = Guid.NewGuid();
        var games = new[] { new Game { Id = gameId, Title = "Test Game" } };
        var query = new GamesQuery { Statuses = new HashSet<GameStatus> { GameStatus.Active } };
    ...
    public async Task GetGames_HandlesEmptyGamesList_ForAnonymousUser()
    {
        ...
        repository.Setup(r => r.GetGames(It.IsAny<PagingData>(), query, Guid.Empty)).ReturnsAsync(Array.Empty<Game>());
