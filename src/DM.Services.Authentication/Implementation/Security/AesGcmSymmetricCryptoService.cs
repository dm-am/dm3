...
/// Version 0 = legacy (no version prefix), Version 1+ = versioned.
/// </summary>
internal class AesGcmSymmetricCryptoService : ISymmetricCryptoService
{
    ...
    private readonly ILogger<AesGcmSymmetricCryptoService> _logger;

    /// <inheritdoc />
    public AesGcmSymmetricCryptoService(
        IOptions<CryptoConfiguration> cryptoOptions,
        ILogger<AesGcmSymmetricCryptoService> logger)
    {
        _logger = logger;
    ...
    public Task<string> Decrypt(string valueToDecrypt)
    {
        ...
        // Try versioned format first (version prefix byte)
        if (combined.Length >= VersionSize + NonceSize + TagSize)
        {
            var version = (int)combined[0];
            if (_allKeys.TryGetValue(version, out var versionedKey))
            {
                try
                {
                    return Task.FromResult(DecryptWithKey(combined, VersionSize, versionedKey));
                }
        ...
        // Legacy format (no version prefix): nonce + tag + ciphertext
        if (combined.Length >= NonceSize + TagSize)
        {
            _logger.LogDebug("Decrypting legacy (unversioned) token");
            return Task.FromResult(DecryptWithKey(combined, 0, _currentKey));
        }
