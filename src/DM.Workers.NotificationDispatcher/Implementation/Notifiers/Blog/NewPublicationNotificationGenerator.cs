...
/// <summary>
/// Generates notifications when a new publication is created in a blog.
/// Notifies blog readers (subscribers).
/// </summary>
internal class NewPublicationNotificationGenerator : BaseNotificationGenerator
{
    private readonly DmDbContext _dbContext;
    private readonly ISubscriptionRepository _subscriptionRepository;

    /// <inheritdoc />
    public NewPublicationNotificationGenerator(
    ...
    public override async IAsyncEnumerable<CreateNotification> Generate(Guid entityId)
    {
        ...
        // Find all users subscribed to this blog with NewPublications setting
        var subscriptions = await _subscriptionRepository.GetByTargetWithSettings(
            SubscriptionTargetType.Blog,
            publicationData.BlogId,
            SubscriptionSettings.NewPublications);

        var subscriberIds = subscriptions
            .Where(s => s.SubscriberId != publicationData.AuthorId) // Don't notify author
