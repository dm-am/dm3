...
internal class NotificationConsumerRetryMiddleware : IConsumerMiddleware
{
    ...
    private const string Queue = "dm.notifications";
        ...
        ILogger<NotificationConsumerRetryMiddleware> logger)
    {
        _retryPolicy = Policy.Handle<Exception>().WaitAndRetryAsync(5,
            ...
            (exception, _) =>
            {
                ...
                logger.LogWarning(exception, "Error processing notifications");
            });
    }
    ...
    /// </summary>
    /// <remarks>
    /// Every event passes through here, which is why the counters are written
    /// here: the warning below was the only trace a failing generator left, and
    /// it reached no dashboard and no rule.
    /// </remarks>
