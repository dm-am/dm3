...
public class PublicationCommentController : ControllerBase
{
    ...
    /// </remarks>
    /// <param name="id">Publication identifier (GUID)</param>
    /// <param name="comment">Comment data with text content</param>
    ...
    [ProducesResponseType(typeof(GeneralError), 404)]
    public async Task<IActionResult> PostPublicationComment(Guid id, [FromBody] Comment comment)
    {
        var result = await _commentApiService.Create(id, comment);
