...
/// <summary>
/// Service for post review (rating) operations
/// </summary>
public interface IPostReviewService
{
    ...
    Task<Review> CreateAsync(CreatePostReview createReview);
    ...
    Task<(IEnumerable<Review> Reviews, PagingResult Paging)> GetListAsync(Guid postId, PagingQuery query);
    ...
    /// <returns>Reviews and paging info</returns>
    Task<(IEnumerable<Review> Reviews, PagingResult Paging)> GetAllAsync(PagingQuery query, PostReviewFilter? filter = null);
    ...
    Task<Review> UpdateAsync(UpdatePostReview updateReview);
