...
/**
 * UserTopicsList — cross-board topics table for the profile "Топики" tab.
 *
...
async function fetchTopics() {
  loading.value = true;
  try {
    const { data } = await forumApi.getAllTopics(apiQuery.value);
  ...
  <div class="user-topics-list">
    ...
    >
      ...
      <template v-if="paging && paging.pages && paging.pages > 1" #footer>
        <Paging
          :paging="paging"
          :to="{ name: 'profile', params: { username, tab: 'topics' } }"
          :use-query="true"
          query-key="number"
        />
      </template>
