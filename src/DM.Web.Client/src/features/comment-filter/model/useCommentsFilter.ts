...
export interface CommentsFilterComposable {
  /** Current filter state (computed from URL) */
  filterState: ComputedRef<CommentsFilterState>;
  /** API search parameters (computed from filterState) */
  searchParams: ComputedRef<CommentsSearchParams>;
...
function createDefaultState(): CommentsFilterState {
  return {
    search: "",
    authors: new Set(),
    createdFromUtc: null,
...
function parseQueryToState(query: LocationQuery): CommentsFilterState {
...
/**
 * Comments filter composable with action-based state management.
 *
 * Architecture:
...
export function useCommentsFilter(): CommentsFilterComposable {
  ...
  const dispatch = (action: CommentsFilterAction) =>
    dispatcher.dispatch(action, reducer, getCurrentState);

  const searchParams = computed<CommentsSearchParams>(() => {
