 ...
 * - SpoilerView: .spoiler-wrapper > .spoiler-head (span) + .spoiler (content)
 * - NsfwView: .nsfw-wrapper > .nsfw-head (span) + .nsfw-spoiler (content) + .nsfw-overlay
 */
...
describe("NsfwView", () => {
  ...
  describe("Collapse/Expand", () => {
    ...
    it("shows content when not collapsed", () => {
      ...
      expect(wrapper.find(".nsfw-spoiler").isVisible()).toBe(true);
    });
    ...
    it("hides content when collapsed", () => {
      ...
      // v-show sets display: none
      expect(wrapper.find(".nsfw-spoiler").isVisible()).toBe(false);
    });
  });
  ...
  describe("18+ Overlay", () => {
    ...
    it("hides overlay after clicking it", async () => {
      const wrapper = mount(NsfwView, {
        props: createMockProps({ collapsed: false }, updateAttributesMock),
      });

      await wrapper.find(".nsfw-overlay").trigger("click");
      expect(wrapper.find(".nsfw-overlay").exists()).toBe(false);
    });
