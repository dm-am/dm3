...
export interface ReviewsFilterComposable {
  /** Current filter state (computed from URL) */
  filterState: ComputedRef<ReviewsFilterState>;
  /** API search parameters (computed from filterState) */
  searchParams: ComputedRef<ReviewsSearchParams>;
  /** Whether any filters are active */
  hasActiveFilters: ComputedRef<boolean>;

  // Actions
  setSearch: (search: string) => void;
  setSort: (sortBy: ReviewSortBy, sortOrder?: "asc" | "desc") => void;
...
type ReviewsFilterAction =
  | { type: "SET_SEARCH"; search: string }
  | { type: "SET_SORT"; sortBy: ReviewSortBy; sortOrder?: "asc" | "desc" }
...
function createDefaultState(): ReviewsFilterState {
  return { ...DEFAULT_FILTER_STATE };
}

function parseQueryToState(query: LocationQuery): ReviewsFilterState {
...
/**
 * Reviews filter composable with action-based state management.
 *
 * Architecture:
 ...
 */
export function useReviewsFilter(): ReviewsFilterComposable {
  ...
  // Dispatch helper
  const dispatch = (action: ReviewsFilterAction) =>
    dispatcher.dispatch(action, reducer, getCurrentState);

  // Convert to API params
  const searchParams = computed<ReviewsSearchParams>(() => {
    const state = filterState.value;
    const params: ReviewsSearchParams = {};
    ...
    params.size = 10; // Reviews use fixed page size
    return params;
  });
