...
test.afterAll(async () => {
  ...
  if (testSubscriptionId && authContext) {
    await authContext.delete(`${API_URL}/v1/users/me/subscriptions/${testSubscriptionId}`);
  }
...
test.describe('Subscriptions API', () => {
  test('should get my subscriptions list', async () => {
    ...
    const response = await authContext.get(`${API_URL}/v1/users/me/subscriptions`);
  ...
  test('should subscribe to a game', async () => {
    ...
    // Subscribe
    const response = await authContext.post(`${API_URL}/v1/users/me/subscriptions`, {
